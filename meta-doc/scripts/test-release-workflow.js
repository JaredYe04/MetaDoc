...
runTest('get-version-info.js - 使用输入版本', () => {
  const output = execSync(
    `node scripts/get-version-info.js "Beta0.13.10"`,
...
runTest('get-version-info.js - 从 package.json 读取版本', () => {
  const output = execSync(
    `node scripts/get-version-info.js ""`,
    { cwd: rootDir, env: testEnv, encoding: 'utf-8' }
  );
...
runTest('get-release-type.js - workflow_dispatch dev', () => {
  const output = execSync(
    `node scripts/get-release-type.js "dev"`,
    { cwd: rootDir, env: { ...testEnv, INPUT_RELEASE_TYPE: 'dev' }, encoding: 'utf-8' }
  );
...
runTest('get-release-type.js - 从 tag 判断', () => {
  const output = execSync(
    `node scripts/get-release-type.js ""`,
    { cwd: rootDir, env: { ...testEnv, GITHUB_REF: 'refs/tags/dev-0.13.10' }, encoding: 'utf-8' }
  );
  if (!output.includes('type=dev')) {
...
runTest('get-release-info.js - dev 版本', () => {
  const output = execSync(
    `node scripts/get-release-info.js "dev" "0.13.10"`,
    { cwd: rootDir, env: testEnv, encoding: 'utf-8' }
  );
...
runTest('get-release-info.js - prod 版本', () => {
  const output = execSync(
    `node scripts/get-release-info.js "prod" "0.13.10"`,
    { cwd: rootDir, env: testEnv, encoding: 'utf-8' }
  );
...
runTest('check-release-exists.js - release 不存在（应该返回 false）', () => {
  ...
  const output = execSync(
    `node scripts/check-release-exists.js "dev-nonexistent-999.999.999"`,
    { cwd: rootDir, env: testEnv, encoding: 'utf-8' }
  );
...
runTest('check-local-build.js - 检查本地构建产物', () => {
  const output = execSync(
    `node scripts/check-local-build.js "0.13.10"`,
    { cwd: rootDir, env: testEnv, encoding: 'utf-8' }
  );
...
runTest('save-release-log.js - 保存发布日志', () => {
  ...
  execSync(
    ...
    { cwd: rootDir, env: { ...testEnv, GITHUB_SHA: 'test-sha', GITHUB_RUN_ID: '123' }, encoding: 'utf-8' }
  );
