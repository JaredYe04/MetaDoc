    ...
    <div class="proofread-content-wrapper">
      <div class="proofread-header">
        ...
        <div class="header-actions">
          ...
          <el-button 
            v-if="fixedErrorsCount > 0"
            @click="handleClearFixed"
          >
      ...
      <div v-else class="proofread-content">
        ...
        <div v-if="proofreadResult" class="error-stats">
          ...
          <el-alert
            v-else
            :title="$t('proofread.errorsFound', { count: displayErrors.length }, `发现 ${displayErrors.length} 个错误`)"
            type="warning"
            :closable="false"
          />
        </div>
                ...
                >
                  <div class="error-header">
                    ...
                    <span class="error-location" :style="locationStyle">
                      {{ $t('proofread.location', { line: error.line, column: error.column }, `第 ${error.line} 行，第 ${error.column} 列`) }}
                    </span>
                  </div>
                    ...
                    <div class="error-suggestions">
                      ...
                      <div class="suggestions-list">
                        <el-tag
                          ...
                          :key="sugIndex"
                          :type="(error.selectedSuggestion || error.suggestion) === suggestion ? 'primary' : 'info'"
                          class="suggestion-tag"
                          :class="{ 'suggestion-selected': (error.selectedSuggestion || error.suggestion) === suggestion }"
                          size="small"
                          effect="plain"
                  ...
                  <div class="error-actions">
                    ...
                    <el-button 
                      size="small"
                      type="success"
                      @click="handleAddToDictionary(index)"
                    >
...
import { ref, computed, watch, onMounted, onBeforeUnmount, nextTick, type WatchStopHandle } from 'vue'
...
const handleProofread = async () => {
  ...
  if (!proofreadResult.value) {
    proofreadResult.value = {
      ...
      text: activeDocument.value.format === 'tex' 
        ? activeDocument.value.tex 
        : activeDocument.value.markdown,
  ...
  try {
            ...
            streamingErrorsWatcher = watch(
              ...
              (streamingErrorList) => {
                ...
                if (streamedNewErrors.length > 0) {
                  ...
                  console.log(`[ProofreadView] 流式更新：新增 ${streamedNewErrors.length} 个错误，总计 ${proofreadResult.value.totalErrors} 个`)
                }
              },
    ...
    if (result.status === 'succeeded') {
      ...
      } else if (result.data && (result.data as any).content && (result.data as any).content.result) {
  ...
  if (error.line > 0 && error.line <= lines.length) {
    ...
    if (error.column > 0 && error.column <= line.length) {
      ...
      if (actualText !== error.text) {
        console.warn(`[ProofreadView] 错误位置不匹配，跳过修复: 期望 "${error.text}"，实际 "${actualText}"`)
...
const handleAddToDictionary = async (index: number) => {
  ...
  try {
    ...
    for (const sameError of sameTextErrors) {
      const errorIndex = proofreadResult.value.errors.findIndex(
        e => e.line === sameError.line && e.column === sameError.column && e.text === sameError.text
      )
      if (errorIndex >= 0) {
    ...
    ElMessage.success(t('proofread.addToDictionarySuccess', `已将 "${error.text}" 添加到词典，已忽略 ${sameTextErrors.length} 个相同错误`))
  } catch (error) {
