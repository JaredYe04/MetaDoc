...
export class LaTeXSectionAdapter implements SectionOptimizerAdapter {
  ...
  private getEditor(): monaco.editor.IStandaloneCodeEditor | null {
    ...
    return editors.find((e: monaco.editor.IStandaloneCodeEditor) => e.getId?.() === this.editorId) ?? null
  }

  async getSectionAtCursor(cursorPosition: { lineNumber: number; column: number }): Promise<SectionInfo | null> {
    ...
    if (outline) {
      ...
      const node = allNodes.find(n => {
        ...
        return nodeTitle === foundTitle || nodeTitle.includes(foundTitle) || foundTitle.includes(nodeTitle)
      })
      if (node) {
        foundPath = node.path
    ...
    if (nodeContent) {
      ...
      let extractedLines = titleLineIndex >= 0 
        ? nodeLines.slice(titleLineIndex + 1)
        : nodeLines
      // 过滤掉 \end{document} 行
      ...
      if (!content) {
        try {
          ...
          if (titleLineIndex >= 0) {
            content = mdLines.slice(titleLineIndex + 1).join('\n').trim()
          } else {
  ...
  async applyContent(sectionInfo: SectionInfo, newContent: string, append: boolean): Promise<void> {
    ...
    const startPos = new monaco.Position(sectionInfo.range.start.line + 1, sectionInfo.range.start.column + 1)
    const endPos = new monaco.Position(sectionInfo.range.end.line + 1, sectionInfo.range.end.column + 1)
    ...
    if (newContent.includes('#') || newContent.includes('**') || newContent.includes('*')) {
      try {
        ...
        latexContent = latexContent.replace(/\\documentclass.*?\\begin\{document\}/s, '').replace(/\\end\{document\}.*/s, '').trim()
      } catch (e) {
