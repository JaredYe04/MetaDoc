...
function autoUpdateVersion(force = false) {
  ...
  for (const commit of newCommits) {
    ...
    if (
      level === 'major' ||
      (maxLevel !== 'major' && level === 'minor') ||
      (maxLevel === null && level === 'patch')
    ) {
      maxLevel = level
    }
  }
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log(
        '  node version-manager.js update           - 根据 Conventional Commits 自动更新版本（已废弃）'
      )
      console.log(
        '  node version-manager.js update --force    - 强制更新版本（即使没有新 commits）（已废弃）'
      )
      console.log('  node version-manager.js increment         - 手动递增 patch 版本号')
      console.log(
        '  node version-manager.js bump <level>      - 手动升级版本 (major/minor/patch/rebuild)'
      )
      console.log('')
