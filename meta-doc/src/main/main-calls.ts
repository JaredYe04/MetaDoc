...
import {
  convertSvgToPdf,
  convertSvgStringToPngFile,
  convertSvgStringToPdfFile
...
function bindFileHandlers(): void {
  ...
  ipcBridge.registerHandle(
    ...
    async (
      event: IpcMainInvokeEvent,
      filePath: string
    ): Promise<{ birthtime: number; mtime: number; size: number } | null> => {
      try {
        if (!fs.existsSync(filePath)) {
          return null
        }
    ...
    async (event: IpcMainInvokeEvent, filePath: string): Promise<void> => {
      try {
        ...
        try {
          ...
          logger.warn('使用 shell.trashItem 删除失败，尝试直接删除:', {
            path: normalizedPath,
            error: trashError
          })
  ...
  ipcBridge.registerHandle(
    ...
    async (
      event: IpcMainInvokeEvent,
      payload: { filePath: string; content: string; encoding?: 'utf8' | 'base64' }
    ): Promise<void> => {
      try {
  ...
  ipcBridge.registerHandle(
    ...
    async (event: IpcMainInvokeEvent, filePath: string, requestId?: string): Promise<string> => {
      ...
      try {
        ...
        const result = await fileConversionService.tryConvertFileToText(
          filePath,
          progressCallback,
  ...
  ipcBridge.registerHandle(
    ...
    async (
      event: IpcMainInvokeEvent,
      payload: { base64String: string; languages?: string[] }
    ): Promise<string> => {
      try {
   ...
   * 根据文件类型类别生成文件过滤器
   */
  function getFileFiltersByCategory(
    category: string,
    i18nT: (key: string, defaultValue?: string) => string
  ): Electron.FileFilter[] {
    ...
    switch (category) {
      case 'all':
        ...
        filters.push(
          {
            name: i18nT('agent.reference.fileTypeCategory.all', 'All Supported Formats'),
            extensions: [
              'txt',
              'md',
              'json',
          ...
          {
            name: i18nT('agent.reference.fileTypeCategory.text', 'Text Files'),
            extensions: ['txt', 'md', 'json', 'xml']
          },
          {
            name: i18nT('agent.reference.fileTypeCategory.document', 'Document Files'),
            extensions: ['pdf', 'docx', 'doc', 'pptx', 'ppt']
          },
          {
            name: i18nT('agent.reference.fileTypeCategory.data', 'Data Files'),
            extensions: ['csv', 'xlsx', 'xls']
          },
          {
            name: i18nT('agent.reference.fileTypeCategory.image', 'Image Files'),
            extensions: ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp']
          },
          {
            name: i18nT('agent.reference.fileTypeCategory.web', 'Web Files'),
...
function bindUtilityHandlers(): void {
  ...
  ipcBridge.registerHandle(
    ...
    async (
      event: IpcMainInvokeEvent
    ): Promise<{ mode: 'dark' | 'light'; accentColor?: string }> => {
  ...
  async function getWindowsAccentColor(): Promise<string | undefined> {
    try {
      ...
      const command =
        'powershell -Command "Get-ItemProperty -Path \'HKCU:\\Software\\Microsoft\\Windows\\DWM\' -Name AccentColor -ErrorAction SilentlyContinue | Select-Object -ExpandProperty AccentColor"'
      ...
      const makeRequest = (
        ...
        return new Promise((resolve, reject) => {
          try {
            ...
            reject(
              new Error(
                `HTTP请求配置失败: ${error instanceof Error ? error.message : String(error)}`
              )
            )
          }
        })
    ...
    async (
      ...
      try {
        ...
        if (childProcess.stdin && !childProcess.stdin.destroyed) {
          ...
          const buffer =
            process.platform === 'win32' ? iconv.encode(input, 'gbk') : Buffer.from(input, 'utf8')
      ...
      try {
        ...
        if (result.status === 'success') {
          ...
          mainWindow?.webContents.send(
            'compile-latex-fail',
            t('main.latex.compileFailed', `Compilation failed, exit code: ${result.exitCode}`, {
...
function bindSystemHandlers(): void {
  ...
  ipcBridge.registerHandle(
    ...
    async (
      event: IpcMainInvokeEvent
    ): Promise<{ os: string; cpu: string; gpu: string; ram: string }> => {
      try {
          ...
          res.on('end', () => {
            if (res.statusCode === 201) {
              try {
                ...
                const firstFile =
                  json.files && (Object.values(json.files)[0] as { raw_url?: string })
  ...
  ipcBridge.registerHandle(
    ...
    async (
      ...
      if (!gistToken) {
        throw new Error(
          'GITHUB_FEEDBACK_GIST_TOKEN is not configured. Please add it to .env (Gist 专用，仅需 gist 权限).'
        )
      }
      if (!owner) {
      ...
      try {
        ...
        throw new Error(
          '上传反馈正文失败，请检查 GITHUB_FEEDBACK_GIST_TOKEN 是否有效且具备 gist 权限。'
        )
      }
      ...
      function buildAttachmentHtml(filename: string, mime: string, contentBase64: string): string {
        ...
        if (isImageMime(mime)) {
          return [
            ...
            '<img src="' +
              dataUrl +
              '" style="max-width:100%;max-height:100vh;object-fit:contain;" alt="' +
              safeName +
              '"/>',
            '</body></html>'
          ].join('')
      ...
      try {
        const attachments = JSON.parse(payload.attachments || '[]') as Array<{
          filename: string
          mime: string
          content_base64: string
        }>
        const isTextMime = (m: string) =>
          /^text\//.test(m) || m === 'application/json' || m === 'application/xml'
        for (let i = 0; i < Math.min(5, attachments.length); i++) {
          const att = attachments[i]
          const content =
            isImageMime(att.mime) || !isTextMime(att.mime)
              ? buildAttachmentHtml(att.filename, att.mime, att.content_base64)
              : Buffer.from(att.content_base64, 'base64').toString('utf8')
          const gistFilename =
            isImageMime(att.mime) || !isTextMime(att.mime) ? 'view.html' : att.filename
...
const chooseSaveFile = async (data: SaveData): Promise<string> => {
  const dateString = new Date().toLocaleString().replace(/:/g, '-').replace('T', '_').split('.')[0]
...
async function handleSimpleTexOCR(params: SimpleTexOCRData): Promise<any> {
  ...
  const arrayBuffer =
    fileBuffer instanceof ArrayBuffer ? fileBuffer : Buffer.from(fileBuffer).buffer
 ...
 */
export async function findWindowWithToolTab(payload: {
  toolType?: string
  route?: string
...
function bindWindowManagementHandlers(): void {
  ...
  ipcBridge.registerOn(
    ...
    (
      _event: IpcMainEvent,
      {
        ...
        tabData,
        insertIndex
      }: { targetWindowId: number; tabData: any; insertIndex?: number }
    ) => {
      try {
        ...
        logger.debug(
          `Tab传递: ${tabData.tab?.id || 'unknown'} from 窗口 ${sourceWindowId} -> 窗口 ${targetWindowId}`
        )
      } catch (error) {
  ...
  ipcBridge.registerHandle(
    ...
    async (
      event: IpcMainInvokeEvent,
      filePath: string
    ): Promise<{ windowId: number | null; tabId: string | null; state: string | null }> => {
      const result = findWindowWithFile(filePath)
      return {
  ...
  ipcBridge.registerHandle(
    ...
    async (event: IpcMainInvokeEvent, payload: { filePath: string; tabId: string }) => {
      ...
      const result = claimFileOpen(payload.filePath, windowId, payload.tabId)
      return {
        success: result.success,
        existingWindowId: result.existingWindowId,
        existingTabId: result.existingTabId,
  ...
  ipcBridge.registerHandle(
    ...
    async (event: IpcMainInvokeEvent, payload: { filePath: string; newTabId: string }) => {
      const windowId = event.sender.id
      return transferFileOwnership(payload.filePath, windowId, payload.newTabId)
    }
  )
  ...
  ipcBridge.registerHandle(
    ...
    async (event: IpcMainInvokeEvent): Promise<string | null> => {
      try {
        ...
        // 缩小到合适尺寸（宽度 240px，保持比例）
        const { width, height } = image.getSize()
        const ratio = 240 / width
        const resized = image.resize({ width: 240, height: Math.round(height * ratio) })
        return resized.toDataURL()
      } catch (error) {
        logger.error('捕获窗口缩略图失败:', error as Error)
        return null
      }
    }
  )
