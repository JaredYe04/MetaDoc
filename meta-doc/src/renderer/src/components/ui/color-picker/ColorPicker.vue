...
watch(
  ...
  (newVal) => {
    ...
    if (color.isValid()) {
      const newColorStr = props.showAlpha ? color.toHex8String() : color.toHexString()
...
<template>
  <Popover :open="isOpen" @update:open="handleOpenChange">
    ...
    <PopoverContent class="w-auto p-3" align="start" :side-offset="4">
        ...
        <div v-if="showAlpha" class="flex items-center gap-2">
          ...
          <div class="flex-1 h-3 rounded-full relative overflow-hidden">
            ...
            <div
              class="absolute inset-0"
              :style="{
                background: `linear-gradient(to right, transparent, ${tinycolor({ h: hsva.h, s: hsva.s, v: hsva.v }).toHexString()})`
              }"
            />
            <Slider
