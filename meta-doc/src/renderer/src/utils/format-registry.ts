...
import type { OutlineTextAdapter } from './outline-adapters';
...
class FormatRegistry {
  ...
  register(config: FormatConfig): void {
    ...
    for (const ext of config.extensions) {
      ...
      if (this.extensionMap.has(normalizedExt) && this.extensionMap.get(normalizedExt) !== config.id) {
        console.warn(`扩展名 ${normalizedExt} 已映射到 ${this.extensionMap.get(normalizedExt)}，将被重新映射到 ${config.id}`);
      }
