...
export function getLlmConfigSelectedModel(config: LlmConfigItem): string {
  ...
  if (t === 'openai-official') return (config['openai-official']?.selectedModel ?? '').trim() || '—'
 ...
 */
export function getLlmConfigDisplayName(config: LlmConfigItem, t: (key: string) => string): string {
  if (config.isDefault) {
