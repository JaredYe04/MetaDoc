...
export function updateLatexImageLinks(
  ...
  updated = updated.replace(graphicsRegex, (match, imagePath) => {
    ...
    // 调试日志
    logger.debug(`updateLatexImageLinks - match: ${match}, imagePath: ${imagePath}, actualPath: ${actualPath}`);
    logger.debug(`映射表大小: ${imageMappings.size}, 映射键示例: ${Array.from(imageMappings.keys()).slice(0, 3).join(', ')}`);
    ...
    if (!mappedPath && !actualPath.startsWith('http://') && 
    ...
    if (actualPath.startsWith('http://localhost:52521/images/') || 
