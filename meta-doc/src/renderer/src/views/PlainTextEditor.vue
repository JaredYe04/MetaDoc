            ...
            <div class="plaintext-layout">
                <div class="plaintext-main" ref="mainContainerRef">
                    <div class="toolbar" :style="{
                        ...
                        <el-tooltip :content="$t('plaintextEditor.toolbar.toggleLineNumbers')" placement="bottom">
...
const refreshContextMenu = async () => {
    articleContextMenuItems.value = await getArticleContextMenuItems({ isPlainTextEditor: true }) as any[];
}
...
const handleTerminalStdoutStream = (_event: any, payload: any) => {
    if (!payload || !payload.invocationId || payload.invocationId !== currentInvocationId.value) return;
...
onMounted(async () => {
    try {
        ...
        eventBus.emit('show-error',
            t('plaintextEditor.init_failed') + e
        );
    }
    finally {
