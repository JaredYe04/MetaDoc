...
const testDatabaseConnection: TestFunction = {
  ...
  fn: async () => {
    ...
    try {
      ...
      throw new Error(`数据库连接测试失败: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
};
  ...
  fn: async (knowledgeFileId?: number) => {
    ...
    try {
      ...
      if (!queryChunksResult.success || queryChunksResult.chunks.length !== chunks.length) {
        throw new Error(`查询数据块失败: 期望 ${chunks.length} 个, 实际 ${queryChunksResult.chunks?.length || 0} 个`);
      }
      ...
      if (!queryVectorsResult.success || queryVectorsResult.vectors.length !== vectors.length) {
        throw new Error(`查询向量失败: 期望 ${vectors.length} 个, 实际 ${queryVectorsResult.vectors?.length || 0} 个`);
      }
      ...
      throw new Error(`数据块和向量测试失败: ${error instanceof Error ? error.message : String(error)}`);
    }
  },
  params: [
    {
...
const testVectorSearch: TestFunction = {
  ...
  fn: async (queryVector?: string) => {
    ...
    try {
      ...
      const chunks = [
        { index: 0, text: '机器学习是人工智能的一个分支', embedding_model: 'bce-embedding-base_v1' },
        { index: 1, text: '深度学习使用神经网络进行学习', embedding_model: 'bce-embedding-base_v1' },
      ...
      for (const result of searchResult.results) {
        if (typeof result.chunkId !== 'number' || 
      ...
      return {
        ...
        performance: {
          ...
          query: {
            ...
            itemsPerSecond: queryResult.chunks ? Math.round((queryResult.chunks.length / queryDuration) * 1000) : 0
          }
        }
      };
    } catch (error) {
