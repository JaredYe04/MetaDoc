...
export function parseCSV(csvString: string, headerRowIndex?: number): any[] {
  ...
  for (let i = finalHeaderRowIndex + 1; i < lines.length; i++) {
    ...
    const finalValues =
      headerDelimiter === delimiter ? values : line.split(headerDelimiter).map((v) => v.trim())
...
export function performAggregation(
  ...
  // 根据字段类型和策略进行分组
  if (
    groupByFieldInfo?.groupByStrategy === 'bucket' &&
    groupByFieldInfo.category === 'numeric-high-cardinality'
  ) {
    // 数值字段分桶
    ...
    groupByFieldInfo?.groupByStrategy === 'date-granularity' &&
    groupByFieldInfo.dateGranularity
  ) {
    // 日期字段按时间粒度分组
    data.forEach((row) => {
 ...
 * 生成固定模板的分析报告（不调用LLM）
 */
function generateTemplateReport(result: DataAnalysisResult, analysisRequest?: string): string {
  ...
  if (Object.keys(result.descriptiveStats).length > 0) {
    ...
    Object.entries(result.descriptiveStats).forEach(([fieldName, stats]) => {
      ...
      if (stats.mean !== undefined)
        report += `- ${isZh ? '均值' : 'Mean'}: ${stats.mean.toFixed(2)}\n`
      if (stats.median !== undefined)
        report += `- ${isZh ? '中位数' : 'Median'}: ${stats.median.toFixed(2)}\n`
      ...
      if (stats.std !== undefined)
        report += `- ${isZh ? '标准差' : 'Std Dev'}: ${stats.std.toFixed(2)}\n`
      report += `\n`
    })
  }
  ...
  if (result.aggregations && result.aggregations.length > 0) {
    ...
    result.aggregations.forEach((agg) => {
      ...
      Object.entries(agg.aggregations).forEach(([fieldName, aggData]: [string, any]) => {
        ...
        if (aggData.avg !== undefined)
          report += `- ${isZh ? '平均值' : 'Average'}: ${aggData.avg.toFixed(2)}\n`
        if (aggData.count !== undefined)
          report += `- ${isZh ? '数量' : 'Count'}: ${aggData.count}\n`
...
使用Markdown格式。`
  ...
  const { handle, done } = createAiTask('生成数据分析摘要', messages, target, 'chat', originKey, {
    stream: true
  })
...
async function loadDataFromFile(filePath: string, signal?: AbortSignal): Promise<string> {
  ...
  if (filePath.includes('__TEST_DATA_DIR__')) {
    ...
    try {
      ...
      if (appRoot) {
        ...
        const hasMetaDoc =
          normalizedRoot.includes('/meta-doc/') ||
          normalizedRoot.endsWith('/meta-doc') ||
          normalizedRoot.endsWith('\\meta-doc')
  ...
  try {
    ...
    const content = (await messageBridge.invoke('read-file-content', resolvedPath)) as string
    return content
  } catch (error) {
  ...
  try {
    ...
    if (error instanceof Error) {
      if (error.message.includes('Maximum call stack size exceeded')) {
        throw new Error(
          '从URL获取数据失败: 响应数据过大，可能导致栈溢出。请尝试使用较小的数据文件或本地文件。'
        )
      }
...
export const dataAnalysisToolCallback: ToolCallback = async (params, signal, onUpdate) => {
  ...
  const headerRowIndex =
    params.headerRowIndex !== undefined ? (params.headerRowIndex as number) : undefined
  ...
  if (!inferredFormat || !supportedFormats.includes(inferredFormat)) {
    return {
      status: 'failed',
      error: i18n.global.t(
        'agent.tool.dataAnalysis.error.invalidFormat',
        `无效的格式，支持: ${supportedFormats.join(', ')}`
      )
    }
  }
    ...
    if (inferredFormat === 'csv') {
      ...
      if (dataSource === 'file' && typeof data === 'string') {
        try {
          ...
          const excelText = (await messageBridge.invoke('convert-excel-to-text', data)) as string
          ...
          for (const line of lines) {
            ...
            if (rowMatch) {
              ...
              const isHeaderRow =
                headerRowIndex !== undefined ? dataRowIndex === headerRowIndex : dataRowIndex === 0
    ...
    onUpdate(
      ...
      },
      {
        ...
        message: i18n.global.t(
          'agent.tool.dataAnalysis.progress.calculating',
          '正在计算描述统计...'
        )
      }
    )
    ...
    if (autoGroupBy || groupByFields.length > 0) {
      onUpdate(
        ...
        },
        {
          percentage: 70,
          message: i18n.global.t(
            'agent.tool.dataAnalysis.progress.aggregating',
            '正在执行聚合分析...'
          )
        }
      )
      ...
      if (groupByFields.length > 0) {
        ...
        fieldsToGroupBy = groupByFields
          ...
          .filter(
            (item): item is { name: string; categoryInfo: FieldCategoryInfo } => item !== null
          )
      } else if (autoGroupBy) {
        // 自动模式：智能选择适合分组的字段
...
使用Markdown格式。`
        ...
        if (summaryResult.targetRef && summaryResult.donePromise) {
          ...
          onUpdate(
            ...
            },
            {
              percentage: 85,
              message: i18n.global.t(
                'agent.tool.dataAnalysis.progress.summarizing',
                '正在生成分析摘要...'
              )
            }
          )
      ...
      onUpdate(
        ...
        },
        {
          percentage: 90,
          message: i18n.global.t(
            'agent.tool.dataAnalysis.progress.generatingTemplate',
            '正在生成分析报告模板...'
          )
        }
      )
    ...
    return {
      status: 'failed',
      error: i18n.global.t(
        'agent.tool.dataAnalysis.error.failed',
        { error: errorMessage },
        `数据分析失败: ${errorMessage}`
      )
    }
...
const DATA_ANALYSIS_TOOL_NAME = 'Data Analysis'
const DATA_ANALYSIS_TOOL_DESCRIPTION =
  'Automatically analyze CSV, JSON data, extract fields, descriptive statistics, aggregations, etc.'
...
export const dataAnalysisToolConfig: AgentToolConfig = {
  ...
  spec: {
    name: 'data-analysis',
    brief:
      'Automatically analyze structured data (CSV, JSON) providing field extraction, descriptive statistics, and aggregations.',
  ...
  inputSchema: {
    ...
    properties: {
      ...
      format: {
        ...
        description:
          '数据格式（支持csv、json、xls、xlsx）。如果使用file模式且未指定format，将根据文件扩展名自动推断'
      },
      dataSource: {
        type: 'string',
      ...
      headerRowIndex: {
        type: 'number',
        description:
          '表头行索引（从0开始），支持CSV、XLS、XLSX格式。如果不指定，将自动智能检测表头位置',
        minimum: 0
      }
    },
