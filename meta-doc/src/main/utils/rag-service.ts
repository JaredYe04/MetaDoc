...
class RAGServiceImpl implements RAGService {
  ...
  async queryKnowledgeBase(
    question: string,
    scoreThreshold = this.DEFAULT_SCORE_THRESHOLD
  ): Promise<string[]> {
    ...
    // 如果查询文本很长，分段处理
    const chunks =
      question.length > this.CHUNK_SIZE
    ...
    if (merged.length === 0) {
      this.logger.warn(
        `知识库查询未返回结果 (阈值: ${scoreThreshold}, 候选数: ${allCandidates.length}, 合并后: ${merged.length})`
      )
  ...
  removeFromIndex(fileBaseName: string): void {
    try {
      ...
      if (deleted) {
        // 从内存中删除
        this.vectorIndex = this.vectorIndex.filter((vec) => !vec.id.startsWith(fileBaseName + '_'))
  ...
  private preprocessText(text: string): string {
    ...
    processed = processed.replace(/[A-Za-z0-9+/=]{30,}/g, (match) => {
      ...
      if (base64Ratio > 0.8 && match.length >= 30) {
        ...
        // 否则认为是 base64，去除
        this.logger.debug(
          `移除 base64 文本块: ${match.substring(0, 50)}... (长度: ${match.length})`
        )
        return ''
      }
          ...
          res.on('end', () => {
            try {
              ...
              if (res.statusCode && res.statusCode !== 200) {
                this.logger.error(`API 请求失败: HTTP ${res.statusCode}`, {
              ...
              if (response.data && response.data[0] && response.data[0].embedding) {
                ...
                if (embedding.length === this.VECTOR_LEN) {
                  ...
                  this.logger.warn(
                    `API 返回的向量维度不匹配: 期望 ${this.VECTOR_LEN}, 实际 ${embedding.length}`
                  )
                  reject(
                    new Error(`向量维度不匹配: 期望 ${this.VECTOR_LEN}, 实际 ${embedding.length}`)
                  )
                }
              } else {
                // 提供更详细的错误信息
                ...
                reject(
                  new Error(
                    `API 响应格式错误: 期望 response.data[0].embedding，但响应结构为 ${JSON.stringify(Object.keys(response))}`
                  )
                )
              }
            } catch (error) {
  ...
  private rerankResults(
    ...
    // 计算BM25分数（用于文本相似度）
    const bm25Scores = this.calculateBM25(
      queryText,
    ...
    return candidates
      .map((r, index) => {
        ...
        const rerankScore =
          r.cosSim * 0.4 + bm25Score * 0.3 + (r.hybridScore || 0) * 0.2 + positionBonus * 0.1
  ...
  getStats() {
    return {
      ...
      enabledDocuments: Object.values(this.vectorInfo).filter((info) => info.enabled !== false)
        .length,
