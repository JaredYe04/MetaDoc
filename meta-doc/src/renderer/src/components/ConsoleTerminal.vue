...
const handleTabCompletion = async (
  ...
  try {
    ...
    if (tabCompletionCandidates.value.length === 0) {
      const entries = await ipcRenderer.invoke('read-directory', dirPath) as Array<{ name: string; path: string; isDirectory: boolean }>;
    ...
    if (tabCompletionCandidates.value.length > 0) {
      // 切换到下一个候选（循环）
      tabCompletionIndex.value = (tabCompletionIndex.value + 1) % tabCompletionCandidates.value.length;
...
const createEditor = async () => {
  ...
  const editor = monaco.editor.create(editorContainer.value, {
    ...
    fontSize: 13,
    fontFamily: '"JetBrains Mono", "Consolas", "Courier New", "Microsoft YaHei Mono", "SimHei", monospace',
  ...
  // 监听键盘事件
  {
    ...
    editor.onKeyDown((e: monaco.IKeyboardEvent) => {
      ...
      if (modifierKey && e.keyCode === monaco.KeyCode.KeyC && currentInvocationId.value) {
        ...
        if (ipcRenderer?.invoke) {
          ipcRenderer.invoke('terminal-send-interrupt', {
    ...
    editor.onKeyDown(async (e: monaco.IKeyboardEvent) => {
      ...
      if (e.keyCode === monaco.KeyCode.Tab && !e.shiftKey && !e.ctrlKey && !e.altKey && !e.metaKey) {
        ...
        if (lastWord && lastWord.length > 0 && words.length > 1) {
          ...
          if (lastWord.includes('/') || lastWord.includes('\\') || lastWord.startsWith('.') || isAbsolute(lastWord)) {
                ...
                if (ipcRenderer?.invoke) {
                  try {
                    ...
                    if (result && result.success && result.resolvedPath) {
                      ...
                      lines.value.push({ id: lineId++, content: `cd: ${targetPath}: ${errorMsg}`, type: 'err' });
                    }
                  } catch (err) {
