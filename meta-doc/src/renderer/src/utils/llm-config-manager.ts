...
function ensureDefaultConfigs(): void {
  const defaultTypes: Array<'ollama' | 'openai' | 'openai-official' | 'deepseek' | 'gemini'> = ['ollama', 'openai', 'openai-official', 'deepseek', 'gemini'];
 ...
 */
async function createConfigFromSettings(): Promise<Omit<LlmConfigItem, 'id' | 'name' | 'createdAt' | 'updatedAt'>> {
...
async function applyConfigToSettingsWithoutBroadcast(config: LlmConfigItem): Promise<void> {
  ...
  if (config.type === 'ollama' && config.ollama) {
    ...
    await setSetting('openaiOfficialEnableMaxTokens', config['openai-official'].enableMaxTokens ?? false);
...
export async function checkWorkspaceModified(): Promise<boolean> {
  ...
  if (snapshot.type === 'ollama' && snapshot.ollama) {
    ...
    if (snapshot.ollama.apiUrl !== current.apiUrl ||
    ...
    if (!current) return true;
    if (snapshot.openai.apiUrl !== current.apiUrl ||
    ...
    if (!current) return true;
    if (snapshot['openai-official'].apiKey !== current.apiKey ||
    ...
    if (!current) return true;
    if (snapshot.deepseek.apiKey !== current.apiKey ||
    ...
    if (!current) return true;
    if (snapshot.gemini.apiKey !== current.apiKey ||
    ...
    if (!current) return true;
    if (snapshot.metadoc.selectedModel !== current.selectedModel ||
 ...
 * 导入配置
 */
export function importConfig(jsonString: string): { success: boolean; config?: LlmConfigItem; error?: string } {
  try {
 ...
 * 导入多个配置
 */
export function importConfigs(jsonString: string): { success: boolean; imported: number; errors: string[] } {
  try {
    const parsed = JSON.parse(jsonString);
