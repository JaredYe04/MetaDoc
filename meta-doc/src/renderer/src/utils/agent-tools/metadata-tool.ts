...
import { ref } from 'vue'
import { generateTitlePrompt, generateDescriptionPrompt, generateKeywordsPrompt } from '../prompts'
...
async function generateTitleWithLLM(
  ...
  return await retryLLMCall(
    async () => {
      ...
      logger.debug(
        `[generateTitleWithLLM] 开始生成标题，outlineJson长度=${outlineJson.length}, prompt长度=${prompt.length}`
      )
      const target = ref('')
      ...
      const { handle, done } = createAiTask('生成标题', messages, target, 'chat', originKey, {
        stream: true
      })
      ...
      logger.debug(
        `[generateTitleWithLLM] 任务完成，target.value长度=${target.value.length}, rawOutput长度=${rawOutput.length}`
      )
...
async function generateDescriptionWithLLM(
  ...
  return await retryLLMCall(
    async () => {
      ...
      logger.debug(
        `[generateDescriptionWithLLM] 开始生成描述，outlineJson长度=${outlineJson.length}, prompt长度=${prompt.length}`
      )
      const target = ref('')
      ...
      const { handle, done } = createAiTask('生成描述', messages, target, 'chat', originKey, {
        stream: true
      })
      logger.debug(
        `[generateDescriptionWithLLM] 任务已创建，handle=${handle}, originKey=${originKey}`
      )
      ...
      const rawOutput = target.value.trim()
      logger.debug(
        `[generateDescriptionWithLLM] 任务完成，target.value长度=${target.value.length}, rawOutput长度=${rawOutput.length}`
      )
      if (!rawOutput || rawOutput.length === 0) {
        logger.warn('LLM返回空结果，可能是API错误、请求被取消或模型返回空内容', {
          targetValue: target.value,
      ...
      if (jsonMatch) {
        try {
          const parseResult = parseJsonWithClean<{ description?: string; content?: string }>(
            jsonMatch
          )
          if (parseResult.success) {
...
async function generateKeywordsWithLLM(
  ...
  return await retryLLMCall(
    async () => {
      ...
      logger.debug(
        `[generateKeywordsWithLLM] 开始生成关键词，outlineJson长度=${outlineJson.length}, prompt长度=${prompt.length}`
      )
      const target = ref('')
      ...
      const { handle, done } = createAiTask('生成关键词', messages, target, 'chat', originKey, {
        stream: true
      })
      ...
      const rawOutput = target.value.trim()
      logger.debug(
        `[generateKeywordsWithLLM] 任务完成，target.value长度=${target.value.length}, rawOutput长度=${rawOutput.length}`
      )
      if (!rawOutput || rawOutput.length === 0) {
      ...
      if (!targetTabId) {
        return {
          ...
          error: createDetailedError(
            ...
            ['metadata工具需要有一个活动的文档才能操作', '可以通过tabId参数指定要操作的文档']
          )
        }
      }
      ...
      if (!doc) {
        return {
          ...
          error: createDetailedError(
            ...
            ['确保文档已正确打开或tabId有效', '可以通过tabId参数指定要操作的文档']
          )
        }
      }
    }
    ...
    if (operation === 'get') {
        ...
        if (field === 'title') {
          ...
          if (typeof value === 'object' && !Array.isArray(value)) {
            ...
            if (metaObj.description !== undefined)
              meta.description = normalizeStringValue(metaObj.description)
            ...
            return {
              status: 'failed',
              error: i18n.global.t(
                'agent.tool.metadata.error.invalidValue',
                'all字段需要对象类型的value'
              )
            }
          }
      ...
      if (!field) {
        return {
          status: 'failed',
          error: i18n.global.t(
            'agent.tool.metadata.error.missingField',
            'generate操作需要field参数'
          )
        }
      }
      ...
      if (!doc.outline) {
        ...
        return {
          status: 'failed',
          error: i18n.global.t(
            'agent.tool.metadata.error.noOutline',
            '文档大纲为空，无法生成元信息'
          )
        }
      }
      ...
      if (field === 'title') {
        ...
        onUpdate(
          ...
          },
          {
            ...
            message: i18n.global.t(
              'agent.tool.metadata.progress.generatingTitle',
              '正在生成标题...'
            )
          }
        )
        ...
        onUpdate(
          ...
          },
          {
            percentage: 50,
            message: i18n.global.t(
              'agent.tool.metadata.progress.generatingDescription',
              '正在生成描述...'
            )
          }
        )
        ...
        onUpdate(
          ...
          },
          {
            percentage: 70,
            message: i18n.global.t(
              'agent.tool.metadata.progress.generatingKeywords',
              '正在生成关键词...'
            )
          }
        )
...
export const metadataToolConfig: AgentToolConfig = {
  ...
  spec: {
    name: 'metadata',
    brief:
      'Access and modify document metadata. Use AI to automatically generate title, description, keywords, etc.',
