...
import fs from 'fs';
import type { FilePath, VectorIndexItem, VectorDatabaseConfig, QueryResult } from '../../types/utils';
 ...
 * @deprecated 现在使用SQLite的vec_distance函数进行向量搜索
 */
export function legacyComputeHashKey(normalizedVec: number[], hashFunc: number, hashBits: number): string {
...
export function legacyOptimizedAnnSearch(
  ...
  if (candidates.length > config.maxCandidates) {
    candidates = legacyQuickPreSort(normalizedQuery, candidates)
