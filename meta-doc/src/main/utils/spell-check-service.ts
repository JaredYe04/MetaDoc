...
function preprocessText(text: string, format: 'text' | 'markdown' | 'latex'): string {
  if (format === 'markdown') {
    ...
    text = text.replace(
      /\\begin\{(?:verbatim|lstlisting|minted|code)\}[\s\S]*?\\end\{(?:verbatim|lstlisting|minted|code)\}/g,
      ''
    )
...
function writeDictionaryFile(words: Set<string>, name: string): string {
  ...
  logger.debug(
    `[writeDictionaryFile] 已写入词典文件: ${dictPath}, 包含 ${wordsArray.length} 个单词`
  )
  // 验证文件是否真的写入成功
  if (fs.existsSync(dictPath)) {
    ...
    logger.debug(
      `[writeDictionaryFile] 验证词典文件: 文件大小 ${fileStats.size} 字节, 实际行数 ${lineCount}`
    )
  ...
  try {
    ...
    if (!fs.existsSync(configPath)) {
      ...
      return {
        ...
        dictionaries: ['metadoc-builtin', 'metadoc-custom']
      }
    }
    ...
    try {
      ...
      logger.debug(
        '[loadCSpellConfig] 从 JSON 文件加载配置成功，字典数量:',
        baseConfig?.dictionaries?.length || 0
      )
    } catch (parseError) {
    ...
    const mergedConfig = {
      ...
      dictionaries: [...(baseConfig.dictionaries || []), ...dynamicConfig.dictionaries]
    }
    ...
    if (mergedConfig.dictionaryDefinitions && Array.isArray(mergedConfig.dictionaryDefinitions)) {
      ...
      logger.debug(
        '[loadCSpellConfig] 最终启用的字典列表（共',
        allDictNames.size,
        '个）:',
        Array.from(allDictNames).slice(0, 20).join(', '),
        allDictNames.size > 20 ? '...' : ''
      )
    ...
    logger.debug(
      '[loadCSpellConfig] 配置合并完成，总字典数量:',
      mergedConfig.dictionaries?.length || 0
    )
    logger.debug('[loadCSpellConfig] 字典列表:', mergedConfig.dictionaries)
    logger.debug(
      '[loadCSpellConfig] 词典定义数量:',
      mergedConfig.dictionaryDefinitions?.length || 0
    )
...
export async function performSpellCheck(params: SpellCheckParams): Promise<SpellCheckResult> {
  ...
  try {
    ...
    const languageId =
      format === 'latex' ? 'latex' : format === 'markdown' ? 'markdown' : 'plaintext'
    ...
    try {
      ...
      if (result?.settingsUsed) {
        ...
        logger.debug(
          '[performSpellCheck] cspell 实际使用的词典定义数量:',
          result.settingsUsed.dictionaryDefinitions?.length || 0
        )
        logger.debug(
          '[performSpellCheck] cspell 实际使用的 language 字段:',
          result.settingsUsed.language
        )
        logger.debug(
          '[performSpellCheck] cspell 实际使用的 locale 字段:',
          result.settingsUsed.locale
        )
        ...
        if (result.settingsUsed.dictionaries && Array.isArray(result.settingsUsed.dictionaries)) {
          ...
          const missingDicts = expectedDicts.filter(
            (dict: string) => !result.settingsUsed.dictionaries.includes(dict)
          )
          if (missingDicts.length > 0) {
            logger.warn(
              '[performSpellCheck] 警告: 以下预期字典未在 cspell 实际使用的配置中:',
              missingDicts
            )
            logger.debug(
              '[performSpellCheck] 实际使用的字典列表:',
              result.settingsUsed.dictionaries.join(', ')
            )
          } else {
          ...
          // 检查语言字典是否被使用
          const languageDicts = ['fr-fr', 'es-es', 'de-de', 'ru-ru']
          const usedLanguageDicts = languageDicts.filter((dict: string) =>
            result.settingsUsed.dictionaries.includes(dict)
          )
    ...
    if (result.issues && Array.isArray(result.issues) && result.issues.length > 0) {
      ...
      logger.debug(
        `[performSpellCheck] cspell 检测到的错误单词（前20个）:`,
        Array.from(errorWords).slice(0, 20).join(', ')
      )

      // 统计有建议和无建议的单词数量
      const wordsWithSuggestions = Array.from(errorWordsWithSuggestions.entries()).filter(
        ([_, count]) => count > 0
      ).length
      const wordsWithoutSuggestions = Array.from(errorWordsWithSuggestions.entries()).filter(
        ([_, count]) => count === 0
      ).length
      logger.debug(
        `[performSpellCheck] 错误单词统计: 有建议 ${wordsWithSuggestions} 个, 无建议 ${wordsWithoutSuggestions} 个`
      )
      ...
      for (const issue of result.issues) {
        ...
        if (needsMapping) {
          errorText =
            issue.text || processedText.substring(originalOffset, originalOffset + originalLength)
          if (errorText && errorText.trim().length > 0) {
            ...
            const searchEnd = Math.min(
              text.length,
              originalOffset + searchRadius + errorText.length
            )
        ...
        // 获取建议列表
        const suggestions =
          issue.suggestions && Array.isArray(issue.suggestions) ? issue.suggestions : []
        ...
        if (suggestions.length === 0) {
          logger.debug(
            `[performSpellCheck] 跳过单词 "${errorText}"，因为无建议（可能是字典未正确使用）`
          )
          continue
        }
        ...
        if (
          ...
          suggestions.some((s: string) => s.toLowerCase() === errorText.toLowerCase())
        ) {
          logger.debug(
            `[performSpellCheck] 跳过单词 "${errorText}"，因为建议列表包含它本身（可能是误报）`
          )
          continue
        }
      ...
      logger.debug(
        `[performSpellCheck] 过滤后剩余 ${issues.length} 个问题（原始 ${result.issues.length} 个）`
      )
      if (issues.length > 0) {
