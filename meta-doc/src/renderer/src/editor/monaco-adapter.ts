...
export class MonacoTextEditorAdapter implements TextEditorAdapter {
  ...
  private async performAsyncSearch(
    ...
    try {
      ...
      // 使用分批搜索，每批10个匹配
      const searchGenerator = this.scanTextForMatchesBatched(
        text,
        normalized,
        lineStarts,
        startOffset,
  ...
  private finishSearch(
    ...
    if (previousIndex >= 0 && findResults.length > 0) {
      ...
      if (previousIndex < findResults.length) {
        ...
        // 优先尝试找到相同文本的匹配
        const sameTextIndex = findResults.findIndex((m) => m.matchText === previousMatchText)
        if (sameTextIndex >= 0) {
  ...
  find(direction: SearchDirection): EditorSearchState {
    ...
    const nextIndex =
      (this.state.currentIndex + delta + this.state.matches.length) % this.state.matches.length
  ...
  insertText(text: string, opts?: { replaceSelection?: boolean }): void {
    ...
    const startPosition = new monaco.Position(range.startLineNumber, range.startColumn)
  ...
  goTo(position: TextPosition): void {
    ...
    const safeColumn = Math.max(1, Math.min(model.getLineMaxColumn(safeLine), position.column))
  ...
  goToRanges(ranges: TextRange[]): void {
    const selections = ranges.map(
      (range) =>
        new monaco.Selection(range.start.line, range.start.column, range.end.line, range.end.column)
    )
    const editor = this.getEditor()
    if (!editor) return
  ...
  applyDecorations(matches: FindResult[], activeIndex: number) {
    ...
    const decorations = matches.map((match, index) => {
      ...
      return {
        ...
        options: {
          className: isActive ? 'md-editor-search-active' : 'md-editor-search-match',
          isWholeLine: false
        }
  ...
  private getWordSeparators(): string {
    try {
      const editor = this.getEditor()
      if (editor) {
        const option = editor.getOption(monaco.editor.EditorOption.wordSeparators)
  ...
  private offsetsToRange(start: number, end: number, lineStarts: number[]): TextRange {
    return {
  ...
  private offsetToPosition(offset: number, lineStarts: number[]): TextPosition {
  ...
  private getLineNumberFromOffset(lineStarts: number[], offset: number): number {
    let low = 0
  ...
  private isWholeWordBoundary(
    ...
    return this.isSeparatorChar(before, separators) && this.isSeparatorChar(after, separators)
  }

  private isSeparatorChar(char: string | undefined, separators: string): boolean {
    if (char === undefined) return true
    if (separators.includes(char)) return true
    return /\s/.test(char)
  }
