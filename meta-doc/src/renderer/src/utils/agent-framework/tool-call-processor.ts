...
function parseSingleToolCall(
  ...
  try {
    ...
    getLogger().debug(`[parseSingleToolCall] 成功解析工具调用 ${index + 1}: toolId=${toolId}`, parameters)
...
function createInvalidToolCall(
  ...
  const errorMessage = context?.suggestion
    ? `${error}\n\n建议：${context.suggestion}`
    : error
 ...
 * @returns 清理后的内容
 */
export function cleanToolCallMarkers(
  content: string,
