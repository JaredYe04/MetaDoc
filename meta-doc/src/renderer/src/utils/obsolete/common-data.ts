...
export const tree_node_schema = {
  ...
  properties: {
    path: {
      type: 'string',
      description:
        "节点的编号路径，根节点为 'dummy'。例如：'1' 表示根节点dummy的第一个子节点，'1.1' 表示1的第一个子节点，'1.1.1' 表示1.1的第一个子节点，以此类推"
    },
    title: {
      type: 'string',
    ...
    title_level: {
      ...
      minimum: 0,
      description:
        '标题的层级，0 表示根节点，1 表示一级标题，2 表示二级标题，依此类推。一般来说当前标题层级是上一层+1，但是可能不止+1，例如从一级跳到三级。'
    },
    children: {
      type: 'array',
...
var current_outline_tree: Ref<DocumentOutlineNode> = ref(
  JSON.parse(JSON.stringify(default_outline_tree))
)
