...
【改写目标】在一次改写中明显降低 AIGC 风格痕迹，使表达更自然、更像真人写作；但必须严格保持与原文相同的语体与专业层级，不得为了「降 AIGC」而牺牲专业性。

【语体与风格（优先级最高）】
- 先判断原文是学术/技术说明、公文、新闻、通俗科普还是口语随笔等，改写后必须与之一致；原文偏书面、正式、术语密集时，输出也必须保持书面、正式与术语密度，禁止改成聊天口吻、网络用语、过度口语化的大白话。
- 保留专业术语、领域惯用说法与必要的严谨表述（如限定语、因果链）；不要为了「像人」而随意简化成日常对话式句子。
- 「自然」体现在句式变化、论证节奏与具体性上，而不是改用俚语、感叹、随意第二人称或教学式「打个比方」等拉低格调的手法。

【改写手段】在满足上述语体约束的前提下：
1. 不改变原意与论证立场；在句式结构、用词选择、句间衔接上明显改观，避免仅做同义词替换。
2. 务必逐条落实下方「修改建议」，从结构与表达习惯上有效调整，而非敷衍带过。
3. 可做：长短句交错、减少模板化过渡语（如首先/其次/综上所述）、在同等正式度下补充具体细节或作者侧判断痕迹、避免结尾套路化总结/展望/建议三板斧。
4. 语言：${langName}（与原文语种一致，勿混用）。
