...
<template>
  <div class="my-tool-display">
    <!-- 根据data和status渲染内容 -->
    <div v-if="displayData.stage === 'processing'">正在处理: {{ displayData.input }}</div>
    <div v-else-if="displayData.stage === 'completed'">结果: {{ displayData.result }}</div>
  </div>
</template>
...
```typescript
const value =
  params.customParam !== undefined ? params.customParam : await getSetting('defaultValue')
```
