...
class FileConversionServiceImpl implements FileConversionService {
   ...
   * 支持的文件类型列表
   */
  public readonly supportedTypes: readonly SupportedFileType[] = [
    'txt',
    'md',
    'pdf',
    'docx',
  ...
  async tryConvertFileToText(
    ...
    try {
      ...
      if (ext === 'txt' || ext === 'md') {
        ...
        text = await this.convertDocToText(filePath, progressCallback, abortSignal, requestId)
      } else if (ext === 'pptx') {
        text = await this.convertPptxToText(filePath, progressCallback, abortSignal, requestId)
      } else if (ext === 'ppt') {
        text = await this.convertPptToText(filePath, progressCallback, abortSignal, requestId)
  ...
  private async convertPdfToText(
    ...
    try {
      ...
      if (parseImages) {
        try {
          // 提取所有图片的OCR结果（按页面分组）
          const imageOcrResults = await this.extractImagesFromPdfAndOCR(
            dataBuffer,
            progressCallback,
            abortSignal,
            requestId
   ...
   * @param requestId 请求ID，用于管理OCR任务池（可选）
   */
  private async extractImagesFromPdfAndOCR(
    pdfBuffer: Buffer,
    progressCallback?: ProgressCallback,
    abortSignal?: AbortSignal,
    requestId?: string
    ...
    try {
      ...
      while ((match = imagePattern.exec(pdfString)) !== null) {
        try {
          ...
          try {
            ...
            if (isValidImage && imageBuffer.length > 100) {
              // 至少100字节
              imageIndex++
      ...
      const ocrPromises = images.map(async ({ pageIndex, imageIndex, imageBuffer }, idx) => {
        ...
        try {
          const { promise, worker } = await ocrService.recognizeFromBufferWithWorker(
            imageBuffer,
            undefined,
            abortSignal
          )
          ...
          logger.warn(
            `PDF第${pageIndex}页图片${imageIndex} OCR失败:`,
      ...
      // 检查是否有因取消而失败的任务
      const hasCancelled = ocrResults.some(
        (result) =>
   ...
   * @param requestId 请求ID，用于管理OCR任务池（可选）
   */
  private async convertDocxToText(
    filePath: FilePath,
    progressCallback?: ProgressCallback,
    abortSignal?: AbortSignal,
    requestId?: string
    ...
    try {
      ...
      if (parseImages) {
        try {
          ...
          // 提取图片OCR结果
          const imageOcrResults = await this.extractImagesFromDocxAndOCR(
            buffer,
            progressCallback,
            abortSignal,
            requestId
          ...
          if (imageOcrResults.length > 0) {
            ...
            const insertInterval =
              paragraphCount > imageCount
                ? Math.max(1, Math.floor(paragraphCount / (imageCount + 1)))
                : 1
   ...
   * @param requestId 请求ID，用于管理OCR任务池（可选）
   */
  private async convertDocToText(
    filePath: FilePath,
    progressCallback?: ProgressCallback,
    abortSignal?: AbortSignal,
    requestId?: string
    ...
    try {
      ...
      // 使用 word-extractor 提取文本
      const extractor = new WordExtractor()
      let document
      try {
        document = await extractor.extract(filePath)
      } catch (extractError) {
        logger.error('WordExtractor提取文本失败:', extractError)
        const errorDetails =
          extractError instanceof Error
            ? `${extractError.message}${extractError.stack ? `\n${extractError.stack}` : ''}`
            : String(extractError)
        throw new Error(`Word解析失败: ${errorDetails}`)
      }
      ...
      // 可选：获取其他部分（脚注、尾注、页眉页脚等）
      // 注意：DOC格式可能不支持所有方法，需要根据实际情况调整
      try {
      ...
      try {
        ...
        // 忽略页眉页脚提取失败
        logger.debug('提取页眉页脚失败:', error)
      }

      // 注意：DOC格式通常不支持图片OCR，因为它是二进制格式
      // 如果需要图片OCR，建议用户使用DOCX格式

      return text
    } catch (error) {
      logger.error('Error parsing DOC:', error)
      const errorMessage = error instanceof Error ? error.message : 'Failed to parse DOC file'
      throw new Error(`DOC转换失败: ${errorMessage}`)
    }
  }
   ...
   * @param requestId 请求ID，用于管理OCR任务池（可选）
   */
  private async extractImagesFromDocxAndOCR(
    docxBuffer: Buffer,
    progressCallback?: ProgressCallback,
    abortSignal?: AbortSignal,
    requestId?: string
    ...
    try {
      ...
      const imageFiles = Object.keys(zip.files).filter(
        (name) => name.startsWith('word/media/') && /\.(jpg|jpeg|png|gif|bmp)$/i.test(name)
      )
      ...
      const validImages = imageBuffers.filter(
        (img): img is { index: number; filename: string; buffer: Buffer } => img !== null
      )
   ...
   * @param requestId 请求ID，用于管理OCR任务池（可选）
   */
  private async convertPptxToText(
    filePath: FilePath,
    progressCallback?: ProgressCallback,
    abortSignal?: AbortSignal,
    requestId?: string
    ...
    try {
      ...
      for (let i = 0; i < slideFiles.length; i++) {
        ...
        if (parseImages) {
          try {
            slideImageTexts = await this.extractImagesFromPptxSlideAndOCR(
              zip,
              slideIndex,
              totalSlides,
              progressCallback,
        ...
        let slideText = ''
        if (textMatch) {
          slideText = textMatch.map((match) => match.replace(/<[^>]*>/g, '')).join(' ')
        }
        ...
        if (slideText.trim() || slideImageTexts.length > 0) {
          ...
          if (slideImageTexts.length > 0) {
            const cleanOcrTexts = slideImageTexts
              .map((ocr) => {
                // 提取OCR文本（去掉图片标识）
   ...
   * @param requestId 请求ID，用于管理OCR任务池（可选）
   */
  private async convertPptToText(
    filePath: FilePath,
    progressCallback?: ProgressCallback,
    abortSignal?: AbortSignal,
    requestId?: string
    ...
    try {
      ...
      try {
        ...
        const errorDetails =
          parseError instanceof Error
            ? `${parseError.message}${parseError.stack ? `\n${parseError.stack}` : ''}`
            : String(parseError)
      ...
      logger.error('Error parsing PPT:', error)
      const errorMessage = error instanceof Error ? error.message : 'Failed to parse PPT file'
      throw new Error(`PPT转换失败: ${errorMessage}`)
    }
  }
   ...
   * @param requestId 请求ID，用于管理OCR任务池（可选）
   */
  private async extractImagesFromPptxSlideAndOCR(
    zip: JSZip,
    slideIndex: number,
    totalSlides: number,
    progressCallback?: ProgressCallback,
    ...
    try {
      ...
      const imageFiles =
        slideImageFiles.length > 0
          ? slideImageFiles.filter(
              (name) => zip.files[name] && /\.(jpg|jpeg|png|gif|bmp)$/i.test(name)
            )
          : Object.keys(zip.files).filter(
              (name) => name.startsWith('ppt/media/') && /\.(jpg|jpeg|png|gif|bmp)$/i.test(name)
            )
      ...
      const imageBuffers = await Promise.allSettled(
        imageFiles.map(async (imageFile, index) => {
          ...
          try {
            ...
            if (
              abortSignal?.aborted ||
              (error instanceof Error && error.message === '操作已取消')
            ) {
              throw error
            }
            logger.warn(
              `提取PPTX图片 ${index + 1} (${path.basename(imageFile)}) Buffer失败:`,
              error
            )
            return null
          }
      ...
      const hasCancelledInExtraction = imageBuffers.some(
        (result) =>
          result.status === 'rejected' &&
          result.reason instanceof Error &&
          result.reason.message === '操作已取消'
      ...
      logger.info(
        `PPTX幻灯片 ${slideIndex} OCR完成，成功识别 ${imageTexts.length}/${validImages.length} 张图片`
      )
  ...
  private async convertExcelToText(filePath: FilePath): Promise<string> {
    try {
      ...
      if (!fs.existsSync(filePath)) {
        ...
        throw new Error(
          `文件不存在: ${filePath}。文件可能已被删除或移动到其他位置，请重新上传文件。`
        )
      }
      ...
      // 其他错误统一处理
      throw new Error(
        `解析Excel文件失败: ${error instanceof Error ? error.message : String(error)}`
      )
    }
  }
