<template>
    ...
    <div class="theme-cards-container">
      ...
      >
        ...
        <div class="theme-card__info">
          ...
          <div v-else-if="theme.type === 'custom' && theme.isDefault" class="theme-card__color" :style="{ color: theme.themeColor }">
            {{ theme.themeColor }}
          </div>
          <div v-else-if="theme.type === 'sync-color'" class="theme-card__color theme-card__color--sync">
            {{ t('setting.themeSyncColor') }}
          </div>
        </div>
  ...
  <el-form label-width="160px" class="settings-form">
    <el-form-item :label="t('setting.contentTheme')">
        <el-select v-model="settings.contentTheme" :placeholder="t('setting.selectContentTheme')"
        @change="handleContentThemeChange">
        <el-option key="auto" :label="t('setting.auto')" :value="'auto'" />
          <el-option v-for="item in contentThemes" :key="item.value" :label="item.label"
          :value="item.value" />
      </el-select>
    </el-form-item>

    <el-form-item :label="t('setting.codeTheme')">
        <el-select v-model="settings.codeTheme" filterable :placeholder="t('setting.selectCodeTheme')"
...
import { predefineColors, presetThemes, contentThemes, codeThemes, customTheme, lightTheme, darkTheme } from '../../utils/themes.js';
...
const fetchOsThemeInfo = async () => {
  try {
    ...
    if (ipcRenderer) {
      const result = await ipcRenderer.invoke('get-os-theme-info') as { mode: 'dark' | 'light', accentColor?: string };
      // 创建新对象以确保响应式更新
  ...
  if (!visible && theme.type === 'custom' && !theme.isDefault) {
    ...
    setTimeout(() => {
      ...
      if (!colorPickerVisibleMap.value[theme.id] && 
          !colorConfirmedMap.value[theme.id]) {
...
const confirmColorEdit = async (theme: ThemeConfig, color: string | null) => {
  ...
  const newColor = colorToHex(color || editingColorMap.value[theme.id] || theme.themeColor || '#ffffff');
...
const selectTheme = async (theme: ThemeConfig) => {
  ...
  if (theme.type === 'custom' && editingColorMap.value[theme.id] && originalColorMap.value[theme.id]) {
...
onMounted(async () => {
  ...
  if (savedThemeId) {
    ...
    if (globalTheme === 'sync') {
      ...
      const customTheme = allThemes.find(t => t.type === 'custom' && t.id !== 'sync' && t.id !== 'sync-color');
      if (customTheme) {
