...
function bindSystemHandlers(): void {
  ...
  // 开源版：应用内反馈已禁用，请通过 GitHub Issues 提交
  ipcBridge.registerHandle(
    'trigger-feedback-workflow',
    async (): Promise<void> => {
      throw new Error(
        'In-app feedback is disabled in the open-source build. Please submit via GitHub Issues: https://github.com/JaredYe04/MetaDoc/issues/new'
      )
    }
  )
