...
function captureOutput(stepName, command, options = {}) {
  ...
  try {
    ...
    if (Object.keys(outputs).length > 0) {
      console.log(`   📤 输出:`, Object.keys(outputs).map(k => `${k}=${outputs[k]}`).join(', '));
    }
    return true;
  } catch (error) {
...
// 验证关键输出
const requiredOutputs = ['version', 'clean_version', 'type', 'tag', 'release_name', 'is_prerelease', 'notes'];
