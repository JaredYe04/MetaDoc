...
**설명**: 이 문서의 단축키는 현재 코드 구현과 대조되었으며, 메인 프로세스나 렌더러 프로세스에서 구현되어 사용 가능합니다.
...
- **기능**: 열려 있는 모든 문서 저장
- **사용 방법**: 먼저 `Ctrl+K` (또는 `Cmd+K`)를 누른 후 `S` 누르기
...
- **기능**: 다음 탭으로 전환
- **사용 방법**: `Ctrl+Tab`을 누르고 있으면 탭 전환 오버레이가 표시되며, 계속 Tab 키를 눌러 선택하거나 직접 클릭 가능
...
- **기능**: 최근에 닫은 탭 다시 열기
- **사용 방법**: 연속 사용 가능, 최근에 닫은 탭을 순서대로 복원 (최대 20개 복원)
...
- **모두 저장**: 먼저 `Ctrl+K`를 누른 후 `S` 누르기 (동시에 누르지 않음)
- **탭 전환**: `Ctrl+Tab`을 누르고 있으면 오버레이가 표시된 후 계속 Tab 키를 눌러 선택

### 단축키 충돌

단축키가 시스템이나 다른 소프트웨어와 충돌하는 경우:

- **시스템 단축키**: 일부 시스템 단축키가 우선할 수 있음
- **다른 소프트웨어**: 충돌하는 소프트웨어를 닫거나 해당 단축키 수정
- **사용자 지정 단축키**: 향후 버전에서 사용자 지정 단축키 지원 가능성
...
5. **빠른 복원**: 실수로 탭을 닫았을 때 Ctrl+Shift+T로 빠르게 복원
...
5. **모두 저장**: Ctrl+K S는 먼저 Ctrl+K를 누른 후 S를 눌러야 함
