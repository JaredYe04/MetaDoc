...
async function svgToPngBuffer(svgPath, width, height) {
  ...
  if (resized.bitmap.width !== width || resized.bitmap.height !== height) {
    throw new Error(
      `尺寸调整失败: 期望 ${width}x${height}，实际 ${resized.bitmap.width}x${resized.bitmap.height}`
    )
  }
...
async function svgToPngFile(svgPath, outputPath, width, height) {
  ...
  if (image.bitmap.width !== width || image.bitmap.height !== height) {
    throw new Error(
      `输出文件尺寸不正确: 期望 ${width}x${height}，实际 ${image.bitmap.width}x${image.bitmap.height}`
    )
  }
}
...
function ensureDocIconsComposed(versionDir) {
  ...
  // 如果所有版本都已存在，跳过合成
  if (
    fs.existsSync(mdDocWhitePath) &&
    fs.existsSync(mdDocBlackPath) &&
    fs.existsSync(texDocWhitePath) &&
    fs.existsSync(texDocBlackPath)
  ) {
...
async function convertSvgToIco(svgPath, outputPath) {
  try {
    ...
    console.log(
      `✓ 已生成 ICO 文件: ${path.basename(outputPath)} (包含尺寸: ${icoSizes.join(', ')}px)`
    )
  } catch (error) {
