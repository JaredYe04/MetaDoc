...
您可以在 **设置 → 快捷键** 中管理全局快捷键：

- **按键方案**：程序提供 Windows、Linux、macOS 三套默认方案，首次启动会根据当前系统自动选择
- **新建/编辑方案**：可创建自定义方案并修改各动作的按键
- **导入/导出**：支持将方案导出为 JSON 文件，或从文件导入方案
- **恢复默认**：每个按键项若与默认方案不同，可点击「恢复默认」还原
