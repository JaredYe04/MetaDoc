...
const displayData = computed(() => {
  ...
  const defaultStage =
    props.status === 'succeeded' ? 'completed' : props.status === 'failed' ? 'error' : 'searching'
  return {
    stage: defaultStage,
    result: undefined,
    error: undefined
...
const hasReplacedContent = computed(() => {
  return !!(
    resultData.value?.replacedContent &&
    resultData.value.replacedCount &&
    resultData.value.replacedCount > 0
  )
})
...
const getMatchItemStyle = (index: number) => {
  ...
  return {
    ...
    border: `1px solid ${
      isActive ? themeState.currentTheme.primaryColor : themeState.currentTheme.textColor2
    }20`,
    borderRadius: '6px',
    padding: '12px',
    marginBottom: '8px',
