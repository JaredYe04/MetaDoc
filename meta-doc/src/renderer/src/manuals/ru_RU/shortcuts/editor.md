...
**Примечание**: Поиск/замена (Ctrl+F, Ctrl+H) реализованы на глобальном уровне приложения; функции жирного/курсива/ссылок/блоков кода и т.д. предоставляются базовым редактором (для Markdown используется Vditor, для LaTeX — Monaco). Если они не работают, ориентируйтесь на фактическое поведение редактора.
...
graph LR
    ...
    A --> F[Сворачивание/разворачивание]
...
- **Полное слово**: Сопоставлять только целые слова
...
- **Сценарий использования**: Обозначение цитат или выделение
...
**Важно**: Это сочетание клавиш может конфликтовать с "Сохранить все" (Ctrl+K S). Необходимо сначала нажать Ctrl+K, а затем K, а не одновременно.
...
### Клавиши автоматического запуска дополнения
...
- **Сценарий использования**: Добавление примеров кода
...
| Вставка блока кода  | `Ctrl+Shift+K`              |
| Свернуть            | `Ctrl+Shift+[`              |
...
| Жирный шрифт        | `Cmd+B`             |
| Курсив              | `Cmd+I`             |
| Вставка ссылки      | `Cmd+K`             |
| Вставка блока кода  | `Cmd+Shift+K`       |
| Свернуть            | `Cmd+Option+[`      |
...
1. **Поиск и замена**: `Ctrl+H` для открытия поиска и замены, затем Tab для переключения между полями ввода.
...
- **Сворачивание**: `[` и `]` соответствуют сворачиванию и разворачиванию.
...
2. **Комбинированные операции**: Сочетайте несколько горячих клавиш для выполнения сложных задач редактирования.
3. **Массовое редактирование**: Используйте функцию многокурсорного редактирования для массового редактирования.
