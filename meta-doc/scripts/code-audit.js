...
function getRecentCommits(months = 6) {
  try {
    ...
    for (const line of lines) {
      ...
      try {
        const statOutput = execSync(`git show --stat --format="" ${hash}`, {
          cwd: repoDir,
          encoding: 'utf-8'
        })
...
function getGitSrcLinesCount(repoDir) {
  try {
    ...
    const filesOutput = execSync(`git ls-files src/`, { cwd: repoDir, encoding: 'utf-8' }).trim()

    if (!filesOutput) {
      // 如果 src/ 下没有文件，尝试 meta-doc/src/
      const filesOutput2 = execSync(`git ls-files meta-doc/src/`, {
        cwd: repoDir,
        encoding: 'utf-8'
      }).trim()
...
function getAllCommitsStats(repoDir) {
  ...
  // 获取所有提交的 hash 和日期
  const logOutput = execSync(`git log --pretty=format:"%H|%ad" --date=iso`, {
    cwd: repoDir,
    encoding: 'utf-8'
  }).trim()
  ...
  for (const line of lines) {
    ...
    try {
      ...
      const showOutput = execSync(`git show --numstat --format="" ${hash}`, {
        cwd: repoDir,
        encoding: 'utf-8'
      }).trim()
      ...
      try {
        const diffOutput = execSync(`git diff --numstat ${hash}^..${hash}`, {
          cwd: repoDir,
          encoding: 'utf-8'
        }).trim()
  ...
  try {
    ...
    for (const period of periods) {
      try {
        ...
        const percentage =
          actualTotalLines > 0 ? (Math.abs(netChanges) / actualTotalLines) * 100 : 0

        console.log(
          `      ✅ ${period.name}: ${periodCommits.length} 次提交, 新增 ${totalInsertions.toLocaleString()} 行, 删除 ${totalDeletions.toLocaleString()} 行, 累计改动 ${totalChanges.toLocaleString()} 行, 净变化 ${netChanges >= 0 ? '+' : ''}${netChanges.toLocaleString()} 行, 净变化占比 ${percentage.toFixed(2)}%`
        )
...
function generateChartExplanation(chartType, data, context = {}) {
  switch (chartType) {
    case 'processPie':
      ...
      const mainPercent = (
        ((data.find((d) => d.name === 'main')?.value || 0) / totalLines) *
        100
      ).toFixed(1)
      const rendererPercent = (
        ((data.find((d) => d.name === 'renderer')?.value || 0) / totalLines) *
        100
      ).toFixed(1)
    ...
    case 'languagePie':
      ...
      const topLangPercent = (
        (topLang.value / data.reduce((sum, d) => sum + d.value, 0)) *
        100
      ).toFixed(1)
      const top3Langs = data
        .slice(0, 3)
        .map((d) => d.name)
        .join('、')
    ...
    case 'complexityBar':
      ...
      const dominant = complexityDistribution.reduce(
    ...
    case 'processLanguagePie':
      ...
      const mainLangPercent = (
        (mainLang.value / data.reduce((sum, d) => sum + d.value, 0)) *
        100
      ).toFixed(1)
    ...
    case 'contributor':
      ...
      const topPercent = (
        (topContributor.value / data.reduce((sum, d) => sum + d.value, 0)) *
        100
      ).toFixed(1)
    ...
    case 'dependencyType':
      ...
      const devDepsPercent =
        totalDeps > 0 ? ((context.devDepsCount / totalDeps) * 100).toFixed(1) : 0
    ...
    case 'dependencyScope':
      ...
      const topScopePercent = (
        (topScope.value / data.reduce((sum, d) => sum + d.value, 0)) *
        100
      ).toFixed(1)
    ...
    case 'timePeriodPie':
      ...
      const maxPeriodPercent = (
        (maxPeriod.value / data.reduce((sum, d) => sum + d.value, 0)) *
        100
      ).toFixed(1)
...
function generateMarkdownReport(stats, outputPath, gitCommits = null, npmDeps = null) {
  ...
  markdown += JSON.stringify(
    generateBarChart('进程代码行数对比', processNames, processLines, '代码行数 (LOC)'),
    null,
    2
  )
  markdown += `\n\`\`\`\n\n`
  markdown += generateChartExplanation(
    'processBar',
    processData.map((d, i) => ({ name: d.name, value: processLines[i] }))
  )
  ...
  const languages = Object.entries(stats.languages).sort((a, b) => b[1].lines - a[1].lines)
  ...
  markdown += `\`\`\`echarts\n`
  markdown += JSON.stringify(
    generateBarChart('各语言文件数对比 (Top 10)', langNames, langFiles, '文件数'),
    null,
    2
  )
  markdown += `\n\`\`\`\n\n`
  ...
  complexityRanges.forEach((range) => {
    const count = stats.complexityDistribution.filter(
      (c) => c >= range.min && c <= range.max
    ).length
    const percentage =
      stats.total.codeFiles > 0 ? ((count / stats.total.codeFiles) * 100).toFixed(2) : '0.00'
  ...
  markdown += `\`\`\`echarts\n`
  markdown += JSON.stringify(
    generateBarChart('圈复杂度分布对比', complexityLabels, complexityValues, '文件数'),
    null,
    2
  )
  markdown += `\n\`\`\`\n\n`
  ...
  if (stats.highComplexityFiles.length > 0) {
    ...
    markdown += `\`\`\`echarts\n`
    markdown += JSON.stringify(
      generateLineChart('高复杂度文件趋势 (Top 10)', fileNames, complexities, '圈复杂度'),
      null,
      2
    )
    markdown += `\n\`\`\`\n\n`
    ...
    markdown += `\`\`\`echarts\n`
    markdown += JSON.stringify(
      generateBarChart('高复杂度文件对比 (Top 10)', fileNames, complexities, '圈复杂度'),
      null,
      2
    )
    markdown += `\n\`\`\`\n\n`
    ...
    if (stats.processes[process] && stats.processes[process].files > 0) {
      ...
      const maxComplexity = Math.max(
        ...processOrder.map((proc) => {
          ...
          return procStats && procStats.codeFiles > 0
            ? procStats.complexity / procStats.codeFiles
            : 0
        })
      )
      ...
      if (processIndicators.length === 0) {
        processIndicators.push(
          ...
          {
            name: '有效代码行数',
            max: Math.max(...processOrder.map((proc) => stats.processes[proc]?.codeLines || 0))
          },
      ...
      processSeriesData.push({
        name: process,
        value: [p.lines, p.files, p.codeLines, p.codeFiles > 0 ? p.complexity / p.codeFiles : 0]
      })
    }
  })
  ...
  if (processIndicators.length > 0) {
    ...
    markdown += `\`\`\`echarts\n`
    markdown += JSON.stringify(
      generateRadarChart('进程多维度分析', processIndicators, processSeriesData),
      null,
      2
    )
    markdown += `\n\`\`\`\n\n`
  ...
  if (processCompositionData.length > 0) {
    ...
    const totalCode = processCompositionData.reduce(
      (sum, d) => sum + d.codeLines + d.emptyLines + d.commentLines,
      0
    )
    ...
    markdown += `\`\`\`echarts\n`
    markdown += JSON.stringify(
      generateStackedBarChart('进程代码组成分析', compositionNames, compositionSeries),
      null,
      2
    )
    markdown += `\n\`\`\`\n\n`
  ...
  processOrder.forEach((process) => {
    if (stats.processes[process] && stats.processes[process].files > 0) {
      ...
      markdown += `|------|--------|----------|\n`
      const processLangData = Object.entries(langStats).sort((a, b) => b[1].lines - a[1].lines)
      ...
      if (processLangData.length > 0) {
        ...
        markdown += `\`\`\`echarts\n`
        markdown += JSON.stringify(
          generatePieChart(`${process} 进程语言分布`, processLangPieData),
          null,
          2
        )
        markdown += `\n\`\`\`\n\n`
        markdown += generateChartExplanation('processLanguagePie', processLangPieData, {
          processName: process
        })
      }
  ...
  if (timePeriodStats) {
    ...
    markdown += `\`\`\`echarts\n`
    markdown += JSON.stringify(
      generateTimePeriodStackedBarChart('时间段改动量对比', timePeriodStats),
      null,
      2
    )
    markdown += `\n\`\`\`\n\n`
    ...
    markdown += `\`\`\`echarts\n`
    markdown += JSON.stringify(
      generateTimePeriodStackedChart('时间段改动量组成分析', timePeriodStats),
      null,
      2
    )
    markdown += `\n\`\`\`\n\n`
    ...
    const avgPercentage = (
      timePeriodStats.reduce((sum, s) => sum + s.percentage, 0) / timePeriodStats.length
    ).toFixed(2)
  ...
  if (npmDeps) {
    ...
    markdown += `\`\`\`echarts\n`
    markdown += JSON.stringify(
      generateDependencyTypeChart('依赖项类型对比', npmDeps.dependencies, npmDeps.devDependencies),
      null,
      2
    )
    markdown += `\n\`\`\`\n\n`
    ...
    if (scopeData.length > 0) {
      ...
      markdown += `\`\`\`echarts\n`
      markdown += JSON.stringify(
        generateDependencyScopeChart('依赖项作用域分布', npmDeps.scopedDeps),
        null,
        2
      )
      markdown += `\n\`\`\`\n\n`
      ...
      markdown += `\`\`\`echarts\n`
      markdown += JSON.stringify(
        generateDependencyScopeBarChart('依赖项作用域对比', npmDeps.scopedDeps),
        null,
        2
      )
      markdown += `\n\`\`\`\n\n`
    ...
    if (hasMultiplePrefixes) {
      ...
      markdown += `\`\`\`echarts\n`
      markdown += JSON.stringify(
        generateVersionPrefixChart('版本前缀分布', npmDeps.versionPrefixes),
        null,
        2
      )
      markdown += `\n\`\`\`\n\n`
      const totalVersions = Object.values(npmDeps.versionPrefixes).reduce(
        (sum, count) => sum + count,
        0
      )
      ...
      const prefixType =
        Object.entries(npmDeps.versionPrefixes).find(([_, count]) => count > 0)?.[0] || 'unknown'
      const prefixName =
        prefixType === '^'
          ? '兼容版本（^）'
          : prefixType === '~'
...
function main() {
  ...
  if (npmDeps) {
    console.log(
      `   找到 ${npmDeps.total} 个依赖项（${npmDeps.dependencies} 个生产依赖，${npmDeps.devDependencies} 个开发依赖）`
    )
  }
  ...
  console.log(
    `   总代码行数: ${stats.total.lines.toLocaleString()} (${(stats.total.lines / 1000).toFixed(2)} KLOC)`
  )
