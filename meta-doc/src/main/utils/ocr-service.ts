...
function cleanOcrText(text: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    line = line.replace(
      /([\u4e00-\u9fa5\u3400-\u4dbf\u3000-\u303f\uff00-\uffef\u2e80-\u2eff\u2f00-\u2fdf\u31c0-\u31ef])\s+([a-zA-Z0-9])/g,
      '$1 $2'
    )
    line = line.replace(
      /([a-zA-Z0-9])\s+([\u4e00-\u9fa5\u3400-\u4dbf\u3000-\u303f\uff00-\uffef\u2e80-\u2eff\u2f00-\u2fdf\u31c0-\u31ef])/g,
      '$1 $2'
    )
  ...
  private async preprocessImage(imageBuffer: Buffer, preprocessingParams?: any): Promise<Buffer> {
    try {
      ...
      if (preprocessingParams) {
        ...
        // 饱和度调整（或灰度化）
        if (
          preprocessingParams.grayscale ||
          (preprocessingParams.saturation !== undefined && preprocessingParams.saturation < 0)
        ) {
          image = image.greyscale()
        } else if (
          preprocessingParams.saturation !== undefined &&
          preprocessingParams.saturation !== 0
        ) {
          const saturation = (preprocessingParams.saturation + 100) / 100
          image = image.color([{ apply: 'saturate', params: [saturation] }])
        }
      ...
      const wrappedPromise = new Promise<string>((resolve, reject) => {
        ...
        recognizePromise
          .then(async (result) => {
            ...
            logger.debug(
              `OCR识别完成，原始文本长度: ${text.length}，清理后长度: ${cleanedText.length}`
            )
          ...
          .catch(async (error) => {
            ...
            logger.error('OCR识别失败:', error)
            reject(
              new Error(`OCR识别失败: ${error instanceof Error ? error.message : String(error)}`)
            )
          })
      })
