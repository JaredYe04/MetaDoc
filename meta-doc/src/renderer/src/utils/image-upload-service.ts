...
async function uploadToLocalService(
  ...
  try {
    ...
    if (options.file) {
      ...
      const fileName = options.fileName || `image-${Date.now()}.${(options.file as File).name?.split('.').pop() || 'png'}`;
    ...
    if (useRelativePath && options.documentPath && (action === 'saveToDocumentDir' || action === 'saveToAssetsDir')) {
      ...
      if (relativePath) {
        ...
        // 如果无法计算相对路径，使用绝对路径
        logger.warn('无法计算相对路径，使用绝对路径', { imagePath, documentPath: options.documentPath });
        // 统一使用正斜杠（跨平台兼容）
