...
function cleanupUpdateCache(): void {
  try {
    ...
    if (fs.existsSync(electronUpdaterCacheDir)) {
      try {
        const cacheDirs = fs
          .readdirSync(electronUpdaterCacheDir, { withFileTypes: true })
              ...
              for (const file of files) {
                ...
                try {
                  ...
                  if (stats.isFile()) {
                    const ext = path.extname(file).toLowerCase()
                    if (
                      ['.exe', '.dmg', '.zip', '.AppImage', '.deb', '.rpm', '.pkg'].includes(ext)
                    ) {
                      fs.unlinkSync(filePath)
...
export function initUpdateService(): void {
  try {
    ...
    logger.info(
      `更新服务已初始化: ${config.githubOwner}/${config.githubRepo}${!app.isPackaged ? ' (开发环境)' : ''}`
    )
  } catch (error) {
