...
export function getKnowledgeFileByFilename(filename: string): KnowledgeFile | undefined {
  ensureInitialized()
  return queryOne<KnowledgeFile>('SELECT * FROM knowledge_files WHERE filename = ?', [filename])
}
...
export function getKnowledgeFileById(id: number): KnowledgeFile | undefined {
  ensureInitialized()
  return queryOne<KnowledgeFile>('SELECT * FROM knowledge_files WHERE id = ?', [id])
}
...
export function getAllKnowledgeFiles(): KnowledgeFile[] {
  ensureInitialized()
  return query<KnowledgeFile>('SELECT * FROM knowledge_files ORDER BY created_at DESC')
}
...
export function updateKnowledgeFile(
  ...
  execute(`UPDATE knowledge_files SET ${fields.join(', ')} WHERE id = ?`, values)
}
...
export function deleteVectorsByFileId(knowledgeFileId: number): void {
  ...
  // 获取该文件的所有 chunk_id
  const chunks = query<{ id: number }>('SELECT id FROM data_chunks WHERE knowledge_file_id = ?', [
    knowledgeFileId
  ])
...
export function createVector(chunkId: number, embedding: number[]): number {
  ...
  const result = execute(`INSERT INTO vectors (chunk_id, embedding) VALUES (?, ?)`, [
    chunkId,
    buffer
  ])
...
export function createVectors(vectors: Array<{ chunkId: number; embedding: number[] }>): number[] {
  ...
  transaction(() => {
    for (const vec of vectors) {
      ...
      execute('UPDATE data_chunks SET vector_id = ? WHERE id = ?', [vectorId, vec.chunkId])
    }
  })
...
export function getVectorByChunkId(chunkId: number): Vector | undefined {
  ensureInitialized()
  return queryOne<Vector>('SELECT * FROM vectors WHERE chunk_id = ?', [chunkId])
}

/**
 * 获取文件的所有向量
 */
export function getVectorsByFileId(
  knowledgeFileId: number
): Array<{ chunkId: number; embedding: number[] }> {
  ensureInitialized()
  ...
  if (isSqliteVecAvailable() && tableExists('vec0_index')) {
    try {
      ...
      const countResult = queryOne<{ count: number }>(
        'SELECT COUNT(*) as count FROM vec0_index',
        []
      )
      ...
      try {
        const sampleResult = queryOne<{ chunk_id: number }>(
          'SELECT chunk_id FROM vec0_index LIMIT 1',
          []
        )
        if (!sampleResult) {
      ...
      logger.debug(
        `KNN查询返回 ${knnResults.length} 个结果，chunk_id列表: ${knnResults
          .slice(0, 10)
          .map((r) => r.chunk_id)
          .join(', ')}`
      )
      ...
      logger.debug(
        `KNN结果中有 ${existingChunks.length}/${knnResults.length} 个chunk_id在data_chunks表中存在`
      )
      ...
      if (finalResults.length > 0) {
        ...
        logger.debug(
          `sqlite-vec查询返回 ${finalResults.length} 个结果，前5个distance值: ${sampleDistances.join(', ')}`
        )
      } else {
      ...
      logger.debug(
        `distance统计: 均值=${meanDistance.toFixed(4)}, 标准差=${stdDistance.toFixed(4)}, 范围=[${minDistance.toFixed(4)}, ${maxDistance.toFixed(4)}]`
      )
      ...
      const resultsWithSimilarity = finalResults
        .map((r) => {
          ...
          let baseSimilarity =
            absoluteSimilarity * absoluteWeight + relativeSimilarity * relativeWeight
          ...
          if (queryText) {
            ...
            logger.debug(
              `chunk_id=${r.chunk_id}, baseSim=${baseSimilarity.toFixed(4)}, keywordSim=${keywordSim.toFixed(4)}, finalSim=${finalSimilarity.toFixed(4)}`
            )
          }
      ...
      const filteredResults = resultsWithSimilarity
        .filter((r) => {
          ...
          if (!passes && resultsWithSimilarity.length <= 10) {
            ...
            logger.debug(
              `结果被过滤: chunk_id=${r.chunkId}, distance=${r.rawDistance.toFixed(4)}, baseSim=${baseSim}, finalSim=${r.similarity.toFixed(4)}, threshold=${threshold}`
            )
          }
          return passes
        })
      ...
      if (filteredResults.length > 0) {
        logger.debug(
          `sqlite-vec虚拟表搜索成功: 找到 ${filteredResults.length} 个结果（阈值=${threshold}）`
        )
        return filteredResults
      } else {
        const sampleSimilarities = resultsWithSimilarity
          .slice(0, 5)
          .map((r) => r.similarity.toFixed(4))
        logger.debug(
          `sqlite-vec虚拟表搜索完成，但没有结果超过阈值（阈值=${threshold}），返回的${resultsWithSimilarity.length}个结果的similarity值: ${sampleSimilarities.join(', ')}`
        )
        return []
      }
    } catch (error) {
...
export function resyncVec0Index(): void {
  ...
  try {
    ...
    transaction(() => {
      for (const vec of vectors) {
        try {
          ...
          failCount++
          logger.debug(
            `插入vec0_index失败: chunk_id=${vec.chunk_id}, 类型=${typeof vec.chunk_id}`,
            error as Error
          )
        }
      }
    ...
    logger.info(
      `vec0_index表统计: 总数=${vec0Count?.count}, chunk_id范围=${vec0Range?.min_id} - ${vec0Range?.max_id}`
    )
  } catch (error) {
