...
const WARNING_LICENSES = ['GPL-3.0', 'AGPL-3.0', 'GPL-3.0-only', 'GPL-3.0-or-later', 'AGPL-3.0-only', 'AGPL-3.0-or-later'];
...
function getLicensePermissiveness(license) {
  ...
  const hasWarningLicense = WARNING_LICENSES.some(wl => {
    const pattern = new RegExp(`\\b${wl.toUpperCase().replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i');
...
try {
  ...
  // 按数量排序许可证
  const sortedLicenses = Object.entries(licenseStats)
    .sort((a, b) => b[1] - a[1]);
