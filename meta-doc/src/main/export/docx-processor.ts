...
import { getFormulaConversionConfig, shouldLogVerbose } from '../utils/formula-conversion-config'
...
export class DocxProcessingManager {
  ...
  async process(buffer: Buffer, options: Record<string, any> = {}): Promise<Buffer> {
    ...
    const documentRelsXml = documentRelsFile ? await documentRelsFile.async('string') : ''
    const contentTypesXml = contentTypesFile ? await contentTypesFile.async('string') : ''
    const settingsXml = settingsXmlFile ? await settingsXmlFile.async('string') : null
    ...
    // 按顺序执行所有处理器
    let modified = false
    const progressCallback = options?.progressCallback as
...
export class WordTocProcessor implements DocxProcessor {
  ...
  async process(
    ...
    const tocTitleRegex =
      /<w:p[^>]*>[\s\S]*?<w:pPr[^>]*>[\s\S]*?<w:jc[^>]*w:val="center"[^>]*\/?>[\s\S]*?<\/w:pPr>[\s\S]*?<w:r[^>]*>[\s\S]*?<w:rPr[^>]*>[\s\S]*?<w:b[^>]*\/?>[\s\S]*?<\/w:rPr>[\s\S]*?<w:t[^>]*>[\s\S]{0,50}(?:目录|Table of Contents|目次|Contents)[\s\S]{0,50}<\/w:t>[\s\S]*?<\/w:r>[\s\S]*?<\/w:p>/i
    ...
    if (tocTitleMatch) {
      ...
      if (
        ...
        placeholderStart >= 0 &&
        placeholderEnd >= 0 &&
        placeholderStart > tocTitleEnd
      ) {
        // 占位符在目录标题之后，先删除它
        updatedXml =
          updatedXml.substring(0, placeholderStart) + updatedXml.substring(placeholderEnd)
      ...
      updatedXml =
        updatedXml.substring(0, tocTitleEnd) + tocFieldXml + updatedXml.substring(tocTitleEnd)
      modified = true
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
    ...
    // 更新关系文件
    const updatedRels = this.updateDocumentRels(relsXml, headerRelId, footerRelId)
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">`
  ...
  <Relationship Id="${footerRelId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/footer" Target="footer1.xml"/>`
    ...
    // 解析现有关系文件并添加新关系
    const relationshipsMatch = relsXml.match(/<Relationships[^>]*>([\s\S]*?)<\/Relationships>/)
  ...
  <Relationship Id="${footerRelId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/footer" Target="footer1.xml"/>`
    ...
    return relsXml.replace(/(<Relationships[^>]*>)([\s\S]*?)(<\/Relationships>)/, `$1${newRels}$3`)
  }
...
export class OMMLInsertionProcessor implements DocxProcessor {
  ...
  async process(
    ...
    // 从options中获取公式占位符数据
    const formulaPlaceholders = options?.formulaPlaceholders as
    ...
    // 验证占位符存在性
    const { existingPlaceholders, missingPlaceholders } = this.validatePlaceholders(
      documentXml,
      formulaPlaceholders
    )
    ...
    // 去重公式
    const { uniqueFormulas, sortedIndices } = this.deduplicateFormulas(
    ...
    if (formulaResults.size === 0 && failedIndices.size > 0) {
      ...
      return this.applyFallbackSolution(
        context,
        documentXml,
        formulaPlaceholders,
        existingPlaceholders
  ...
  private validatePlaceholders(
    ...
    if (missingPlaceholders.size > 0) {
      ...
      if (missingPlaceholders.size > 0) {
        for (const missing of missingPlaceholders) {
          ...
          if (placeholderData) {
            logger.debug(
              `示例缺失占位符 ${missing}: LaTeX="${placeholderData.latex}", display=${placeholderData.display}`
            )
            // 查找 XML 中是否有类似的文本
            const searchKey = `MATH_PLACEHOLDER_${missing}`
            const foundInXml = documentXml.includes(searchKey)
            logger.debug(
              `占位符 ${missing} 的核心部分 "${searchKey}" 在 XML 中${foundInXml ? '存在' : '不存在'}`
            )
          }
        }
      }
    ...
    logger.info(
      `验证完成：${existingPlaceholders.size} 个占位符存在于 XML 中，${missingPlaceholders.size} 个不存在`
    )
  ...
  private deduplicateFormulas(
    ...
    for (const index of sortedIndices) {
      ...
      if (verbose) {
        logger.debug(
          `[去重公式] 索引: ${index}, LaTeX长度: ${latexCode.length}, 块级: ${isBlockLevel}`
        )
  ...
  private async convertFormula(
    ...
    conversionCache: Map<
      string,
      { wrappedContent: string; ommlContent: string; tag: string | null }
    >
  ...
  private validateOMML(ommlContent: string, latexCode: string): void {
    try {
      ...
      throw new Error(
        `OMML 不符合 XML 规范: ${validationError instanceof Error ? validationError.message : String(validationError)}`
      )
    }
  }
  /**
  ...
  private enhanceOMMLWithFonts(omml: string): string {
    ...
    enhanced = enhanced.replace(/<m:r([^>]*)>([\s\S]*?)<\/m:r>/g, (match, attrs, content) => {
      // 查找 <m:t> 标签中的文本内容
    ...
    // 匹配 <m:rPr>...</m:rPr>
    enhanced = enhanced.replace(/<m:rPr([^>]*)>([\s\S]*?)<\/m:rPr>/g, (match, attrs, content) => {
      // 检查是否已有 w:rPr
      if (match.includes('<w:rPr')) {
        ...
        if (content.includes('<m:sty m:val="p"')) {
          ...
          // 普通文本，确保字体设置正确（替换现有的）
          return match.replace(/<w:rPr[^>]*>[\s\S]*?<\/w:rPr>/, fontRPr)
        }
      }
    ...
    // 2. 处理自闭合的 <m:rPr/>
    enhanced = enhanced.replace(/<m:rPr([^>]*)\/>/g, (match, attrs) => {
   ...
   */
  private wrapOMMLAsWordprocessingML(
    enhancedOMML: string,
    isBlockLevel: boolean,
    ...
    if (isBlockLevel) {
      ...
      if (tag) {
        ...
        // 左侧 tab：n 个
        const leftTabs = Array(n)
          .fill(
            '<w:r xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:tab/></w:r>'
          )
          .join('')

        // 右侧 tab：n-1 个，然后标签通过右对齐制表位定位
        const rightTabs = Array(n - 1)
          .fill(
            '<w:r xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:tab/></w:r>'
          )
          .join('')
  ...
  private async convertFormulas(
    ...
    for (const [cacheKey, indices] of uniqueFormulas.entries()) {
      ...
      logger.debug(
        `[转换公式] cacheKey长度: ${cacheKey.length}, LaTeX长度: ${latexCode.length}, 块级: ${isBlockLevel}`
      )
        ...
        .then((result) => {
          ...
          if (progressCallback) {
            ...
            progressCallback(
              conversionProgress,
              100,
              `正在转换公式 (${completedCount}/${uniqueCount})`
            )
          }
        })
        .catch((error) => {
          ...
          if (progressCallback) {
            ...
            progressCallback(
              conversionProgress,
              100,
              `正在转换公式 (${totalProcessed}/${uniqueCount})`
            )
          }
        })
    ...
    for (const { textNode, index, isBlockLevel, parentElement } of placeholderLocations) {
      ...
      if (placeholderData) {
        if (verbose) {
          ...
          logger.debug(
            `[替换占位符] 索引: ${index}, LaTeX内容: ${JSON.stringify(placeholderData.latex)}`
          )
        }
      } else {
...
export class ImageNumberingProcessor implements DocxProcessor {
  ...
  async process(
    ...
    const fontFamily =
      options.imageLabelFontFamily || (options.locale === 'en_US' ? 'Arial' : 'SimHei')
    const prefix = options.imageLabelPrefix || (options.locale === 'en_US' ? 'Graph' : '图')
    const defaultName =
      options.imageLabelDefaultName || (options.locale === 'en_US' ? 'Image' : '图片')
    ...
    for (let i = imageParagraphs.length - 1; i >= 0; i--) {
      ...
      if (imageName === defaultName && i < imageParagraphs.length - 1) {
        ...
        const altTextMatch = betweenContent.match(
          /<w:t[^>]*data-image-alt-text[^>]*>([^<]*)<\/w:t>/i
        )
        ...
        const afterContent = updatedXml.substring(
          imageParaEnd,
          Math.min(updatedXml.length, imageParaEnd + 500)
        )
    ...
    `
      ...
      // 在图片段落后插入编号标签段落
      updatedXml =
        updatedXml.substring(0, imagePara.endIndex) +
        labelParagraph +
...
export class DocumentXmlFixProcessor implements DocxProcessor {
  ...
  async process(context: DocxProcessingContext): Promise<boolean> {
    ...
    if (tocTitleMatch) {
      ...
      // 查找直到遇到下一个标题或正文开始
      const tocItemsEndRegex =
        /<\/w:p>\s*(?=<w:p[^>]*>[\s\S]*?(?:<w:pStyle[^>]*w:val="Heading|目录结束标记))/
      ...
      if (tocItemsEndMatch) {
        ...
        const checkContent = updatedXml.substring(
          Math.max(0, tocItemsEndPos - 100),
          Math.min(updatedXml.length, tocItemsEndPos + 500)
        )
        const hasPageBreak =
          checkContent.includes('<w:pageBreakAfter') ||
        ...
        if (!hasPageBreak) {
          ...
          updatedXml =
            updatedXml.substring(0, tocItemsEndPos) +
            pageBreakPara +
            updatedXml.substring(tocItemsEndPos)
          modified = true
        ...
        if (paragraphsAfterToc && paragraphsAfterToc.length > 0) {
          ...
          const checkContent = updatedXml.substring(
            Math.max(0, lastParaEndPos - 100),
            Math.min(updatedXml.length, lastParaEndPos + 500)
          )
          const hasPageBreak =
            checkContent.includes('<w:pageBreakAfter') ||
            checkContent.includes('<w:pageBreakBefore') ||
          ...
          if (!hasPageBreak) {
            const pageBreakPara = '<w:p><w:pPr><w:pageBreakAfter/></w:pPr></w:p>'
            updatedXml =
              updatedXml.substring(0, lastParaEndPos) +
              pageBreakPara +
              updatedXml.substring(lastParaEndPos)
            modified = true
