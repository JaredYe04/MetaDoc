...
// 监听图表变化 - 确保立即渲染
watch(() => [props.message.chartMarkdown, props.message.code, props.isStreaming], async (newVal, oldVal) => {
  // 如果正在流式输出，不渲染
...
<template>
  ...
  >
    <el-avatar class="avatar-with-mask" v-if="role !== 'user'" :src="(themeState.currentTheme as any).AiLogo"></el-avatar>
    <!-- 用户消息的操作按钮（在左侧） -->
    ...
    >
      ...
      <div v-else-if="role === 'user' || (!hasChart && !isStreaming)" class="text-content">
        <MdPreview
          ...
          :autoFoldThreshold="300"
        />
        <pre v-else style="white-space: pre-wrap; word-wrap: break-word; margin: 0;">{{ content }}</pre>
      </div>
    </div>
    <!-- AI消息的操作按钮（在右侧） -->
    <transition name="fade">
