  ...
  return await new Promise((resolve, reject) => {
    const check = () => {
      try {
        ...
        if (!el) {
          el = container.querySelector('.katex-html') || container.querySelector('.vditor-reset .katex-html');
        }
...
export async function renderMarkdownMathToImages(markdown, output = 'png') {
  ...
  async function uploadBlob(fileName, blob) {
    ...
    const resp = await fetch('http://localhost:52521/api/image/upload?keepName=1', { method: 'POST', body: form });
