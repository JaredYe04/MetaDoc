...
**在生成文章内容之前，必须先查看当前文档的大纲树结构**，了解是否已有内容，然后决定如何处理：

1. **查看大纲树**：使用 \`outline-tree\` 工具获取文档的大纲结构
   - 检查文档是否已有章节和内容
   - 了解文档的当前状态和结构

2. **根据情况决定操作**：
   - **如果文档已有内容**：
     - 先使用 \`outline-optimize\` 工具的 \`deleteNodes\` 或 \`clearOutline\` 操作清理不需要的节点
     - 或者保留现有内容，在此基础上进行扩展和优化
   - **如果文档为空**：
     - 可以直接开始生成新内容
     - 使用 \`outline-optimize\` 工具生成大纲和内容

3. **避免的错误**：
   - ❌ **不查看大纲就生成**：不要假设文档是空的，必须先查看大纲树
   - ❌ **忽略已有内容**：如果文档已有内容，应该先清理或保留，而不是直接覆盖

**标准流程**：
\`\`\`
用户请求生成文章 → 调用outline-tree查看大纲 → 分析现有内容 → 决定清理或保留 → 生成新内容
\`\`\`
...
8. **内容质量保障**：使用校对工具检查并修正文章中的错误，确保内容的专业性、准确性和可读性。
   - **推荐使用全文或段落比对**：
     ...
     - 校对特定章节：使用 \`{"nodePath": "1.1"}\`（可以先通过 \`outline-tree\` 查看章节路径）⭐
...
2. ❌ **生成文章前不查看大纲**：生成文章前必须先使用 \`outline-tree\` 查看文档大纲，了解是否已有内容，然后决定清理或保留
3. ❌ **总是从行1列1插入**：必须先定位位置，推荐使用grep工具搜索关键词确定位置
