...
**Канал поставки**: если вы пользуетесь MetaDoc через **Steam**, сначала прочитайте раздел **Steam / MetaDoc Cloud** в **[[settings.llm|Настройка LLM]]** (пополнение, баланс, смена модели). Только если вам нужен **собственный API стороннего провайдера**, включите **экспериментальное подключение** в **экспериментальных опциях**, затем продолжайте ниже и см. **[[settings.llm-types|Типы провайдеров LLM]]**.
