  ...
  try {
    ...
    if (chineseRegex.test(code)) {
      ...
      if (originalChineseCount !== encodedChineseCount) {
        logger.warn(
          `中文字符数量不匹配（原始: ${originalChineseCount}, 编码后: ${encodedChineseCount}），使用原始代码`
        )
        encodedCode = code
      }
    }
              ...
              if (chartType === 'mindmap' || chartType === 'markmap') {
                ...
                allElements.forEach((el) => {
                  ...
                  if (el.hasAttribute('class')) {
                    ...
                    el.setAttribute(
                      'style',
                      currentStyle + '; animation: none !important; transition: none !important;'
                    )
                  }
                })
...
export async function preRenderAllCharts(md, cdn, format = '', progressCallback) {
  ...
  for (const [chartType, config] of Object.entries(CHART_TYPES)) {
    ...
    while ((match = regex.exec(md)) !== null) {
      ...
      logger.debug(
        `提取到 ${chartType} 代码块，代码长度: ${code.length}，前50字符: ${code.substring(0, 50)}`
      )
  ...
  const renderTasks = allMatches.map(async ({ chartType, fullMatch, code, config }, index) => {
    ...
    try {
      logger.debug(
        `开始渲染 ${chartType} 图表 ${index + 1}/${allMatches.length}，代码长度: ${code.length}`
      )
      let imageUrl
      if (config.useIpc) {
        if (chartType === 'echarts') {
          ...
          if (
            ...
            logger.error(
              `PlantUML 代码包含 XML 标签，跳过渲染。代码前200字符: ${cleanCode.substring(0, 200)}`
            )
