...
import { suggestionCompletionPrompt } from '../utils/common/prompts'
import { ai_types, cancelAiTask, createAiTask } from '../utils/ai/ai_tasks'
...
function handleSelectionChange() {
  ...
  if (zwsp && range.startContainer !== zwsp && range.startContainer !== suggestionEl) {
    cancelSuggestion()
  }
}
onMounted(() => {
  ...
  ghostDecoration = editor.deltaDecorations(ghostDecoration || [], [
    {
      ...
      options: {
        inlineClassName:
          themeState.currentTheme.type === 'dark' ? 'ai-suggestion-dark' : 'ai-suggestion',
...
function handleVditorKeydown(e) {
  ...
  // 定义需要忽略的控制键
  const ignoredKeys = new Set(['Control', 'Shift', 'Alt', 'Meta', 'CapsLock', 'Insert', 'Delete'])
...
function handleMonacoKeydown(e) {
  ...
  const suggestionControlKeys = new Set(['Escape', 'Tab'])
