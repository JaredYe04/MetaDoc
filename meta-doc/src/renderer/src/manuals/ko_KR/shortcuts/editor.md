...
**설명**: 찾기/바꾸기(Ctrl+F, Ctrl+H)는 애플리케이션 전역에서 구현됩니다. 굵게/기울임꼴/링크/코드 블록 등은 기본 편집기(Markdown은 Vditor 사용, LaTeX는 Monaco 사용)에서 제공하며, 작동하지 않을 경우 실제 편집기 동작을 기준으로 하세요.
...
1. **찾기 및 바꾸기**: `Ctrl+H`로 찾기 및 바꾸기를 열고, Tab 키로 입력 상자 전환
2. **텍스트 서식 지정**: 텍스트 선택 후 `Ctrl+B` 또는 `Ctrl+I`로 서식 지정
3. **일괄 편집**: `Ctrl+Shift+L`로 동일한 모든 단어를 선택한 후 통일하여 편집
...
2. **조합 작업**: 여러 단축키를 결합하여 복잡한 편집 완료
3. **일괄 편집**: 다중 커서 기능을 사용하여 일괄 편집
4. **빠른 서식 지정**: 단축키를 사용하여 텍스트 빠르게 서식 지정
5. **찾기 및 바꾸기**: 찾기 및 바꾸기 기능을 사용하여 효율성 향상

## 주의사항

1. **플랫폼 차이**: Windows/Linux는 Ctrl 사용, macOS는 Cmd 사용
2. **단축키 충돌**: 일부 단축키는 편집기 기능과 충돌할 수 있음
3. **컨텍스트 의존성**: 일부 단축키는 특정 컨텍스트에서만 유효함
4. **편집기 차이**: Markdown과 LaTeX 편집기에서 지원하는 단축키가 다를 수 있음
5. **AI 자동 완성**: Shift+Tab은 수동 트리거이며, 자동 트리거는 트리거 키 구성 필요
