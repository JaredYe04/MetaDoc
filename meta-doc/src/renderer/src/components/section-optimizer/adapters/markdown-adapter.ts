  ...
  async getSectionAtCursor(cursorPosition: { line: number; column: number }): Promise<SectionInfo | null> {
    ...
    for (let i = currentLine; i >= 0; i--) {
      ...
      if (match) {
        ...
        logger.debug('[MarkdownSectionAdapter] ✓ 找到标题:', {
          行号: titleLine,
          级别: titleLevel,
          标题: foundTitle,
          原始行: line.trim()
    ...
    if (foundTitle && titleLine >= 0) {
      ...
      if (candidateNodes.length > 0) {
        ...
        logger.debug('[MarkdownSectionAdapter] ✓ 精确匹配成功，选择节点:', {
        ...
        if (fuzzyMatch) {
          ...
          logger.debug('[MarkdownSectionAdapter] ✓ 模糊匹配成功:', {
            行号: fuzzyMatch.titleLine,
            标题: fuzzyMatch.title,
            路径: foundPath
          })
        } else {
          logger.debug('[MarkdownSectionAdapter] ✗ 模糊匹配也失败')
        }
      }
    }
    ...
    for (let i = titleLine + 1; i < lines.length; i++) {
      ...
      if (match) {
        ...
        logger.debug(`  [${i}] 行找到标题，级别:${level}, 内容:${line.trim().substring(0, 50)}`)
        if (level <= titleLevel) {
          endLine = i - 1
          logger.debug('[MarkdownSectionAdapter] ✓ 找到结束位置，行号:', endLine)
          break
        }
      }
    }
    ...
    if (node?.text) {
      ...
      if (titleIndex >= 0) {
        ...
        if (nodeLines.length > 0 && nodeLines[0].match(/^(#{1,6})\s+/)) {
          ...
          logger.debug('[MarkdownSectionAdapter] 直接使用整个文本，内容长度:', finalContent.length)
        }
      }
      // 如果提取的内容为空，使用从全文提取的内容
      if (!finalContent) {
        logger.debug('[MarkdownSectionAdapter] 节点内容为空，使用从源码提取的内容')
        finalContent = extractedContent
      }
    } else {
      // 如果节点没有text，使用从源码提取的内容
      logger.debug('[MarkdownSectionAdapter] 节点不存在或没有text，使用从源码提取的内容')
      finalContent = extractedContent
    }
    ...
    logger.debug('[MarkdownSectionAdapter] 范围:', result.range)
