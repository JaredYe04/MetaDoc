...
class FileConversionServiceImpl implements FileConversionService {
   ...
   * 支持的文件类型列表
   */
  public readonly supportedTypes: readonly SupportedFileType[] = ['txt', 'md', 'pdf', 'docx', 'doc', 'pptx', 'ppt', 'xlsx', 'xls'] as SupportedFileType[];
  ...
  private isSupported(extension: string): extension is SupportedFileType {
    const supportedExtensions: string[] = ['txt', 'md', 'pdf', 'docx', 'doc', 'pptx', 'ppt', 'xlsx', 'xls'];
   ...
   * @param requestId 请求ID，用于管理OCR任务池（可选）
   */
  private async convertDocToText(filePath: FilePath, progressCallback?: ProgressCallback, abortSignal?: AbortSignal, requestId?: string): Promise<string> {
    try {
      // 检查是否已取消
      if (abortSignal?.aborted) {
        throw new Error('操作已取消');
      ...
      try {
        ...
        const errorDetails = extractError instanceof Error 
   ...
   * @param requestId 请求ID，用于管理OCR任务池（可选）
   */
  private async convertPptToText(filePath: FilePath, progressCallback?: ProgressCallback, abortSignal?: AbortSignal, requestId?: string): Promise<string> {
    try {
      // 检查是否已取消
      if (abortSignal?.aborted) {
        throw new Error('操作已取消');
